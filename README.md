...
- **Sessions**
  ...
  - **Intermittent summaries**: an enable switch (on by default), trigger interval / event count, concurrent summary limit, per-run LLM timeout, model source, model name, and Thinking level for periodic session-list summaries. Model source supports Claude / Deepseek / Codex and default Thinking is `low`. With an empty model, Claude uses Haiku, Deepseek uses Sonnet, and Codex delegates directly to the model in `config.toml` without a hidden environment override. Each run freezes a persisted event-revision and rebuild-epoch boundary, then builds a bounded read-only evidence snapshot from recent user intent, validated continuation-checkpoint projection when available, the previous display summary, and recent activity/tool results. The card keeps a one-line headline while Summary history can show up to four concrete lines for progress, next step, and risks; provider fallbacks are labeled as degraded. Codex 0.144.1 still cannot attest that its final model-visible built-in tool registry is empty, so periodic Codex summaries run only inside an empty temporary working directory with a read-only sandbox, no network or base config, empty MCP/dynamic-tool/root inputs, disabled executable features, and timeout abort. This is a weaker boundary than a proven-empty tool registry. Summary settings and reads remain independent of checkpoint generation: summaries never create or update a continuation checkpoint, and disabling summaries starts no new summary-model calls.
  - **Continuation Context**: an automatic-checkpoint switch (on by default), normal refresh interval (default 30 minutes; range 5–1,440), generator provider / free-text model / provider-aware Thinking level, and a raw-user-history ceiling (default 64,000 tokens; range 8,000–128,000). The generator is independent of the per-handoff target adapter/model. Empty-model defaults are Claude Sonnet, Deepseek Sonnet, and the model in Codex `config.toml`; Thinking defaults to `medium`. Codex supports `low` / `medium` / `high` / `xhigh` / `max` / `ultra`; Claude and Deepseek support `low` / `medium` / `high` / `xhigh` / `max`.
    - **Refresh lifecycle**: the normal path requires at least 8,000 newly uncheckpointed normalized tokens, the configured interval since the checkpoint/session baseline, provider idle, and 60 seconds without persisted activity. At 48,000 newly uncheckpointed normalized tokens, the safety path bypasses interval, quiet, and provider-idle gates and queues background work without interrupting the active provider turn. The estimate revision is only an eligibility observation: when the globally serialized job actually starts, it atomically captures the latest durable revision, so in-place tool updates that happened while queued are coalesced instead of disappearing behind an unreconstructible historical boundary. Failures back off for five minutes, and foreground hand-off / recovery takes priority over same-session queued or running background work.
