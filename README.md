...
- **Per-session model controls and unified Continuation Context**: every in-app new-session surface accepts a free-text model id and an adapter-aware Thinking dropdown. Editing an active SDK session's model or Thinking level automatically saves the selection for subsequent turns without interrupting the current reply. UI hand-off, MCP `hand_off_session`, and missing-provider-history recovery now share one provider-neutral Continuation Context engine. It captures an immutable SQLite event-revision snapshot, folds a canonical evidence-backed checkpoint, retains a token-bounded tail of eligible user inputs, and renders the authoritative continuation instruction without recursively nesting older generated capsules. Checkpoint generation can degrade honestly through prior-checkpoint, raw-only, and instruction-only modes. The full provider prompt remains private to main; persisted successor history stores the instruction and continuation lineage.
...
- **Sessions**
  ...
  - **Intermittent summaries**: trigger interval / trigger event count / concurrent summary limit / per-run LLM timeout / model source, model name, and Thinking level for periodic session-list summaries. Model source supports Claude / Deepseek / Codex; default Thinking is `medium`. An empty Claude or Deepseek model uses that provider's Haiku alias, while an empty Codex model delegates directly to the model configured by Codex (without a hidden environment-variable override). Each run freezes a persisted event-revision and rebuild-epoch boundary, then builds a bounded read-only evidence snapshot from recent user intent, validated continuation-checkpoint projection when available, the previous display summary, and recent activity/tool results. The card keeps a one-line headline while Summary history can show up to four concrete lines for progress, next step, and risks; provider fallbacks are labeled as degraded. Codex 0.144.1 still cannot attest that its final model-visible built-in tool registry is empty, so periodic Codex summaries run only inside an empty temporary working directory with a read-only sandbox, no network or base config, empty MCP/dynamic-tool/root inputs, disabled executable features, and timeout abort. This is a weaker boundary than a proven-empty tool registry. These settings and reads remain independent of continuation checkpoint generation: periodic summaries never create or update a checkpoint.
  - **Continuation Context**: the “续接检查点生成器” provider, free-text model, and provider-aware Thinking level (default `high`), plus a raw-history retention ceiling measured in tokens (default 64,000; integer range 8,000–128,000). The generator is independent of the per-handoff target adapter/model. When the model field is empty, Claude uses Opus, Deepseek uses Sonnet, and Codex delegates directly to its configured default model. Codex supports `low` / `medium` / `high` / `xhigh` / `max` / `ultra`; Claude and Deepseek support `low` / `medium` / `high` / `xhigh` / `max`. Codex compact generation uses the same hardened-but-unattested empty-cwd/read-only/no-network/no-MCP boundary as periodic summaries, then accepts output only after schema canonicalization, exact evidence allowlisting, active-fact carry-forward checks, and revision-safe persistence. Completed tool telemetry is conservatively deduplicated and compacted with bounded prefix/suffix evidence plus hashes so high-volume tool output cannot starve ordinary message evidence; UI and MCP hand-off share a 300-second checkpoint deadline, while missing-history recovery keeps its separate 30-second limit. Provider or validation failure falls back to the last valid checkpoint plus immutable raw history. Raw retention, target prompt capacity, checkpoint projection, and generator fold input are four distinct budgets; no message-count setting controls capacity.
