...
- **Embedded human responses** (SDK sessions only): tool permission requests, Claude proactive questions (options / other / notes), Claude Plan mode execution-plan approval, and MCP `request_plan_review` plan review are all handled directly in activity-stream cards. When approving Claude Plan mode, you can choose the target permission mode (default / auto-accept edits / keep Plan / fully bypass prompts); switching to "fully bypass prompts" automatically restarts the SDK child process.
- **OS-level sandbox**: Claude Code / Deepseek (Claude Code) SDK child processes can use `workspace-write` or `strict` isolation (macOS Seatbelt / Linux bubblewrap). cwd is writable, sensitive directories such as `~/.ssh` are unreadable, and network is denied by default. When a model wants network access, the `SandboxNetworkAccess` tool loop catches it and prompts the user to retry with `dangerouslyDisableSandbox: true`; the user sees only one final approval dialog. This is aligned with Codex child-process `workspace-write` isolation. The **three-part control surface** is fully symmetric with Codex: (1) global default in Settings, defaulting to workspace-write; (2) new-session override in NewSessionDialog with four choices, "follow settings / off / workspace-write / strict"; (3) runtime switching inside a session from the dropdown above the SessionDetail input, where `off` asks for confirmation and `workspace-write` / `strict` apply directly through a 5-10s cold SDK restart.
- **Universal Team Backend**: cross-adapter (`claude-code` / `deepseek-claude-code` / `codex-cli`) sessions deliver cross-adapter team messages through DB envelopes + universal-message-watcher. `mcp__agent-deck__spawn_session(team_name)` adds both the lead and teammates to the named team; the Team tab can manually add existing active sessions to an existing team; `mcp__agent-deck__send_message` uses the DB queue and automatically injects replies into the lead conversation. **Messages can be sent without a shared team**: when caller and target have no shared active team, `send_message` automatically falls back to a teamless DM (the message is still injected into the target session, but it does not appear in the team aggregate panel). **Inbox-only teams created inside the CLI are not visible in the agent-deck UI**; use `mcp__agent-deck__spawn_session` to create teams in the universal backend.
...
- **Model token statistics**: the top bar center shows real-time "output tokens/s" for today's top 3 most-used models over a recent 60-second sliding window; when the window is too narrow it degrades by hiding down to Top1 and then hiding entirely. The Data tab, alongside Live / Pending / History / Team / Issues, shows each model's daily token use: a model x date table (input / output / cache read / cache write), today's summary, a live all-model token/s area, and the current quota window, weekly usage, and reset time for Claude / Codex accounts. At app startup, local token details are prefetched and provider quota caches are warmed; when the Data tab opens, it reuses cache and refreshes on a timer. Open Claude / Codex sessions are reused first; when no live session exists, an independent silent quota probe starts with the app's private directory, without loading Claude hook settings, creating an Agent Deck session, or sending user messages / model turns. If the provider requires interactive auth, the probe fails and returns. Deepseek currently shows unsupported because it uses the API channel. Token usage is collected from each assistant message (Claude) / completed turn (Codex). Variants of the same base model (thinking / 1m and similar) are merged by friendly name. When no model is specified explicitly, Claude / Codex default-model placeholders are displayed.
- **Command-line entry**: `agent-deck new --cwd ... --prompt ...` starts a new session from any terminal.
- **Bundled app-level conventions + skill / agent injection**: every in-app SDK session automatically appends bundled CLAUDE.md / CODEX_AGENTS.md to the system prompt. Bundled prompt / agent / skill resources are self-contained inside the app bundle; user-defined agents/skills are an enhancement layer. Agent Deck can inject its bundled `deep-review` skill plus native heterogeneous `reviewer-claude` (Claude Code adapter) / `reviewer-codex` (Codex SDK adapter) adversarial reviewers.
- **Multiple adapters**: Claude Code (hook + SDK channels, with SDK streaming input for multi-turn interaction) + Deepseek (Claude Code protocol-adapter channel, independent `~/.agent_deck/.deepseek/settings.json` for URL / token / model while reusing Claude-side agents/skills/CLAUDE.md) + Codex CLI (app-server SDK channel, turn-based protocol that waits for each turn to finish; during an active normal turn, mid-turn steering can inject user corrections into the current turn; Codex reasoning summaries appear in the activity stream, but raw reasoning content is not shown. Codex is often used as a reviewer / subtask teammate; choose it for lead sessions according to personal preference).
...
- **Deepseek (Claude Code)**: reuses the Claude Code SDK bridge and Claude-side agents/skills/CLAUDE.md; only auth and model env are overridden from `~/.agent_deck/.deepseek/settings.json`; no independent hook is installed.
- **Codex CLI**: SDK channel based on `codex app-server --stdio`, with create / send message / interrupt / resume / active-turn correction. It does not support tool approval / proactive questions / Plan mode / runtime permission-mode switching because the Codex protocol does not support them. Normal messages are still turn-based and wait for the previous turn to finish. The Codex adapter's `sendMessage` automatically uses `turn/steer` when the current normal turn is busy; otherwise it queues a message for the next turn. The same input box switches to "correction" mode based on state.

To add an adapter, implement and register the `AgentAdapter` interface.

### Diff Rendering

`DiffRegistry` + `DiffRendererPlugin` interface. Built-ins: text (uses Monaco DiffEditor when before/after snapshots exist; when Codex or old records only provide unified diff, it reconstructs before/after snippets and reuses the same Monaco DiffEditor; only binary/rename patches that cannot be parsed are shown as raw text; the Changes page's "final diff" is based on file before/after snapshots captured in session records and no longer reads the current Git worktree; when historical records lack snapshots, it falls back to recorded patches) / image (side-by-side / after-only / slider comparison views) / pdf (placeholder). Register new renderers in `src/renderer/components/diff/install.ts`.
...
- **Deepseek (Claude Code)**: the first time you create this session type, Agent Deck automatically creates `~/.agent_deck/.deepseek/settings.json`; fill in `env.ANTHROPIC_AUTH_TOKEN` and it is ready to use. The file independently stores DeepSeek `ANTHROPIC_BASE_URL` / token / model names and default Fable / Opus / Sonnet / Haiku alias mappings. agents / skills / CLAUDE.md / MCP still reuse Claude Code-side resources.
...
- **Sessions**
  - **Lifecycle**: active window in minutes / closed threshold in hours / permission-request timeout in minutes (default 30, 0 = no timeout, timeout means deny + interrupt) / history-session retention days / **disconnect-recovery injected message count** (default 30; when jsonl is missing and the app resumes through a fresh CLI/thread, it injects the latest N raw DB conversation messages in addition to the LLM summary; budgeted concatenation means the actual count can be <= configured value) / resolved issue / soft-deleted issue retention days / **cross-session message retention days** (default 30, 0 = disabled; pending/delivering in-flight messages are never deleted)
  - **Intermittent summaries**: trigger interval / trigger event count / concurrent summary limit / per-run LLM timeout / model source, model name, and Thinking level for periodic summaries and Hand-off briefs. Model source supports Claude / Deepseek / Codex. When model name is empty, the settings page hints the current source's default model (Claude / Deepseek periodic summaries default to Haiku, Hand-off defaults to Sonnet, Codex uses the configured default model). Thinking level only affects Codex.
- **Alerts And Appearance**
  ...
  - **Window**: transparent while pinned (see the desktop underneath; on by default. When off, the pinned window still uses real macOS under-window glass; Windows and other platforms have no vibrancy effect) / launch at login
...
- **Integration And Runtime**
  ...
  - **Experimental features**:
    - **Claude Code sandbox**: three-choice dropdown (Off / Workspace Write / Strict, default Workspace Write); only applies on macOS (Seatbelt) / Linux (bubblewrap). **Windows currently does not support OS-level sandboxing**; the settings panel shows platform-specific descriptions. Common tools (git / pnpm / npm / yarn / bun / pip / cargo / go) are exempt by default. This value is the global default; the new-session dialog can override it per session; a session can switch at runtime through a cold restart.
    - **Codex sandbox**: three-choice dropdown (Workspace Write / Read Only / Danger Full Access), aligned with Claude's default.
- **Cross-Tool Collaboration (MCP)**
  - **Agent Deck MCP server (on by default)**: after enabling it, claude / deepseek / codex / third-party MCP clients can orchestrate other coding-agent sessions across adapters, request user plan reviews, manage structured tasks, and report issues through 17 tools (7 session / interaction: `spawn_session` / `send_message` / `request_plan_review` / `list_sessions` / `get_session` / `shutdown_session` / `hand_off_session`; 2 worktree: `enter_worktree` / `exit_worktree`; 5 task: `task_create` / `task_list` / `task_get` / `task_update` / `task_delete`; 3 issue: `report_issue` / `append_issue_context` / `update_issue_status`). Transport:
...
│   │   ├── deepseek-claude-code/ Claude Code SDK profile wrapper; injects DeepSeek env from ~/.agent_deck/.deepseek/settings.json and reuses Claude-side resources
│   │   └── codex-cli/     codex app-server JSON-RPC bridge (pendingMessages serial turn + interrupt + steer)
│   ├── agent-deck-mcp/    Agent Deck MCP server (17 tools + in-process / HTTP transport + spawn-guards / rate-limiter)
