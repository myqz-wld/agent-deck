...
`DiffRegistry` + `DiffRendererPlugin` 接口。内置 text（有 before/after 快照时用 Monaco DiffEditor；Codex 或最终 diff 仅提供 unified diff 时会先还原成 before/after 片段复用同一 Monaco DiffEditor，解析不了的 binary/rename patch 才显示原文；改动页可切到当前 Git 工作区的文件最终 diff，非 Git 目录会退化为会话记录 + 当前文件快照还原）/ image（侧并排 / after-only / 滑动对比三视图）/ pdf（占位）。新增渲染器在 `src/renderer/components/diff/install.ts` 注册即可。
