...
A universal cockpit for Coding Agents. It is a translucent frosted-glass floating window that aggregates multiple Claude Code, Deepseek (Claude Code), Codex CLI, and Grok Build sessions, showing activity, file-change diffs, and periodic summaries in real time. When any session hands control back to you, Agent Deck immediately alerts you with color, sound, and a system notification.
...
- **Bundled app-level conventions + skill / agent injection**: every in-app session receives its adapter-native bundled baseline: Claude `systemPrompt.append`, Codex `developerInstructions`, or Grok ACP `_meta.rules` plus `_meta.pluginDirs`. Grok keeps `_meta.agentProfile` separate, so a named bundled Agent and the app convention remain active together. Bundled prompt / agent / skill resources are self-contained inside the app bundle; user-defined agents/skills are an enhancement layer. Agent Deck can inject its bundled review skills plus heterogeneous reviewer slots: `reviewer-claude` (Claude Code), `reviewer-codex` (Codex SDK), `reviewer-deepseek` (Deepseek v4-pro), and `reviewer-grok` (Grok Build). Both skills use exactly two confirmed slots per run: `simple-review` performs one review and one rebuttal before returning the decision to the user, while `deep-review` iterates autonomously and pauses for user review only when remediation requires a major design decision.
- **Multiple adapters**: Claude Code (hook + SDK channels) + Deepseek (Claude Code protocol-adapter channel and Claude-side assets) + Codex CLI (hook + app-server SDK channels) + Grok Build (official ACP v1 over one bounded `grok agent --no-leader stdio` child per active Agent Deck session). Before Grok `session/new` or `session/load`, Agent Deck reads the advertised ACP `authMethods`, tries API-key auth and cached login in order, and calls `authenticate`; credentials, native tools, sessions, plugins, and permissions remain owned by the Grok CLI. Agent Deck does not scrape the TUI, parse private session files, bundle the Grok binary, store API keys, or modify `~/.grok`.
...
The scheduler scans active+dormant sessions every few minutes and triggers by elapsed time or event count. It has three fallback layers: run one low-cost LLM oneshot with the provider selected in settings -> use the latest assistant text -> use event-kind statistics. The provider is selected independently of the session adapter, so Grok sessions continue to use the configured Claude / Deepseek / Codex summary provider; this integration does not add a second Grok-specific oneshot architecture. The LLM writes a one-sentence description of "what the session is currently doing", shown on the second line of cards and in the SessionDetail Summary tab.
...
- Packaged assets remain immutable. In the Assets Library, only bundled Agents expose **Edit**: model and thinking can be overridden for Claude/Deepseek, Codex, and Grok; Codex Agents can additionally select or enter a native `model_provider`. Bundled Skills remain read-only, while user/project Agents retain their existing full native editing behavior.
...
Claude Code / Codex / Grok Build paths do not read or write API keys. The Deepseek path only reads the token you put in `.deepseek/settings.json` and injects it into the corresponding SDK child process; it does not write it anywhere else.
...
agent-deck new \
  ...
  [--agent claude-code|deepseek-claude-code|codex-cli|grok-build]  # short names claude|deepseek|codex|grok are supported; --adapter is equivalent
...
- **Cross-Tool Collaboration (MCP)**
  - **Agent Deck MCP server (on by default)**: after enabling it, authenticated Claude / Deepseek / Codex / Grok Build callers can orchestrate other coding-agent sessions across adapters, present plans and diffs to users for confirmation or feedback, manage structured tasks, and report issues through 19 tools (7 session / messaging: `spawn_session` / `send_message` / `list_sessions` / `get_session` / `list_session_events` / `shutdown_session` / `hand_off_session`; 2 user presentation: `present_plan` / `present_diff`; 2 worktree: `enter_worktree` / `exit_worktree`; 5 task: `task_create` / `task_list` / `task_get` / `task_update` / `task_delete`; 3 issue: `report_issue` / `append_issue_context` / `update_issue_status`). The runtime profile filters tool visibility and schema variants from the authenticated caller adapter; Grok Build, for example, receives fresh-only `contextMode` because its current ACP surface does not provide native fork. Third-party MCP clients can connect through the external transport, with available tools limited by the external-caller restrictions below. `spawn_session.model` and `hand_off_session.model` accept maintained suggestions grouped by adapter: Claude (`haiku`, `sonnet`, `opus`, `fable`), Codex (`gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.5`, `gpt-5.4`), Deepseek (`v4-flash`, `v4-pro`), and Grok Build (`grok-4.5`). These suggestions are not an allowlist: any non-empty provider model id is passed through for provider validation; only the two Deepseek aliases expand to `deepseek-v4-flash` and `deepseek-v4-pro[1m]`. Transport:
...
Settings has General, Claude Code, Codex CLI, and Grok Build views. Claude and Codex expose their user-scope terminal Hooks; Grok exposes native config guidance plus an ACP authentication probe because it has no Agent Deck terminal Hook. The bottom of General has **Reset to default configuration**; it restores configurable preferences across all four views while preserving installation authentication tokens and installed terminal Hooks.
...
│   │   └── grok-build/    official ACP v1 bridge over bounded `grok agent --no-leader stdio` children
