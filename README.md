...
- **Per-session model controls and compact hand-off**: every in-app new-session surface accepts a free-text model id and an adapter-aware Thinking dropdown. An active SDK session can persist a new model / thinking level for subsequent turns. Hand-off can also choose the successor adapter, model, and thinking level; it creates a fresh versioned context capsule containing a compressed checkpoint, a bounded tail of recent raw user/assistant messages, and a final continuation instruction. If checkpoint generation fails, the UI keeps the raw conversation and marks the preview as degraded instead of discarding the hand-off context.
...
- **Sessions**
  - **Lifecycle**: active window in minutes / closed threshold in hours / permission-request timeout in minutes (default 30, 0 = no timeout, timeout means deny + interrupt) / history-session retention days / **recovery and hand-off raw-message candidate count** (default 200; disconnect recovery and UI/MCP hand-off read at most the latest N raw DB conversation messages, then apply their own character budgets, so the actual injected count can be lower) / resolved issue / soft-deleted issue retention days / **cross-session message retention days** (default 30, 0 = disabled; pending/delivering in-flight messages are never deleted)
  - **Intermittent summaries**: trigger interval / trigger event count / concurrent summary limit / per-run LLM timeout / model source, model name, and Thinking level for periodic summaries and Hand-off briefs. Model source supports Claude / Deepseek / Codex. When model name is empty, the settings page hints the current source's default model (Claude / Deepseek periodic summaries default to Haiku, Hand-off defaults to Sonnet, Codex uses the configured default model). Thinking values are provider-scoped: Codex supports `minimal` / `low` / `medium` / `high` / `xhigh` / `max` / `ultra`; Claude and Deepseek pass Claude Code effort values `low` / `medium` / `high` / `xhigh` / `max`. Actual Claude effort availability depends on the selected model, and Claude Code may downgrade an unsupported level.
