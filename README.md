...
A universal cockpit for Coding Agents. It is a translucent frosted-glass floating window that aggregates multiple Claude Code, Codex CLI, and Grok Build sessions—including Claude Gateway profiles such as Deepseek—showing activity, file-change diffs, and periodic summaries in real time. When any session hands control back to you, Agent Deck immediately alerts you with color, sound, and a system notification.

It is built for people who "drive multiple coding agents at once": if you have Claude Code, Codex, and Grok Build sessions running together, you no longer need to switch terminal windows one by one to see who is waiting. The window floats in the corner of the desktop, and a red flashing badge plus an alert sound tells you which session stopped.
...
- **Bundled app-level conventions + skill / agent injection**: every in-app session receives its adapter-native bundled baseline: Claude `systemPrompt.append`, Codex `developerInstructions`, or Grok ACP `_meta.rules` plus `_meta.pluginDirs`. Grok keeps `_meta.agentProfile` separate, so a named bundled Agent and the app convention remain active together. Bundled prompt / agent / skill resources are self-contained inside the app bundle; user-defined agents/skills are an enhancement layer. Agent Deck can inject its bundled review skills plus heterogeneous reviewer slots: `reviewer-claude` (Claude Code), `reviewer-codex` (Codex SDK), and `reviewer-grok` (Grok Build). Both skills use exactly two confirmed slots per run: `simple-review` performs one review and one rebuttal before returning the decision to the user, while `deep-review` iterates autonomously and pauses for user review only when remediation requires a major design decision. A Deepseek run is an ordinary explicit Claude runtime selection (`provider=deepseek` plus a model), not a bundled reviewer Agent.
- **Multiple adapters**: Claude Code (hook + SDK channels with per-session Gateway settings profiles) + Codex CLI (hook + app-server SDK channels with per-thread `model_provider`) + Grok Build (external native Hook + official ACP v1 over one bounded `grok agent --no-leader stdio` child per active Agent Deck session). Before Grok `session/new` or `session/load`, Agent Deck reads the advertised ACP `authMethods`, tries API-key auth and cached login in order, and calls `authenticate`; credentials, native tools, sessions, plugins, and permissions remain owned by the Grok CLI. Agent Deck does not scrape the TUI, parse private session files, bundle the Grok binary, store API keys, or modify `~/.grok/config.toml`, `~/.grok/AGENTS.md`, or user plugins.
...
- **Internal (SDK/ACP channel)**: created from the plus dialog or `agent-deck new`; the app starts the provider through its structured SDK or ACP interface. Events are marked `source: 'sdk'`; tool calls can be intercepted, sessions can be interrupted, and sessions can be resumed.
- **External (Hook channel)**: run `claude` / `codex` / `grok` directly in a terminal; installed hooks POST events to the embedded HTTP server (default `127.0.0.1:47821`, Bearer-token auth). Events are marked `source: 'hook'`; they are read-only and cannot be controlled from the app. Grok uses its native `~/.grok/hooks/agent-deck.json` file rather than terminal scraping or private session-file parsing.

SDK-owned sessionIds are added to `sdkOwned`, and hook-channel events with the same id are dropped to avoid duplicate display. Agent Deck-managed Grok children also export `AGENT_DECK_ORIGIN=sdk`; native Hook events from those children are dropped even when the ACP and native session ids differ.
...
The `AgentAdapter` interface declares `capabilities`, and the UI hides unsupported fields based on capability. A typed runtime-profile registry is the shared source for adapter capabilities, prompt/resource injection, model and thinking controls, native work modes, and Agent Deck MCP tool policy. Authenticated HTTP MCP callers are resolved back to their persisted session adapter; model-supplied adapter fields are never trusted for tool selection.

- **Claude Code**: hook + SDK channels, with all capabilities enabled (create / interrupt / send message / tool approval / AskUserQuestion / ExitPlanMode / switch permission mode / install hook). Each SDK child may independently load one discovered `~/.claude/gateways/<profile>.json` through `options.settings`, so simultaneous sessions can use different Gateways without mutating global environment state.
...
- **Grok Build**: official ACP v1 over stdio plus a native external terminal Hook. Agent Deck launches `grok agent --no-leader stdio`, negotiates capabilities and authentication, calls ACP `authenticate`, then creates or loads the native session. On macOS/Linux, the real Grok child starts through a supported user login shell so GUI launches can pass exported credential variables directly to Grok without Agent Deck reading or persisting them; shell startup output is kept off the ACP JSON stream. The bridge maps structured text/thinking/tool/file/plan/permission/usage events, uses Grok's `x.ai/interject` extension for ordinary input during an active turn, keeps handoff/continuation input FIFO, and falls back to queued prompts when the extension is unavailable. Grok's `_x.ai/session/update` turn usage feeds token statistics, while the Data tab imports native history on demand and shows live token/s estimates during generation. The Agent Deck session id stays stable while the Grok native session id remains separate. Default / Plan / Ask are Grok work modes, not Claude permission modes. Installing `~/.grok/hooks/agent-deck.json` captures terminal Grok sessions as read-only records. Managed ACP/headless children disable Grok's Claude/Cursor compatibility scanners and tag any remaining native Hook callbacks as SDK-owned, while current Agent Deck Claude Hook commands ignore Grok compatibility invocations.
...
The scheduler scans active+dormant sessions every few minutes and triggers by elapsed time or event count. It has three fallback layers: run one low-cost LLM oneshot with the configured adapter, runtime provider, model, and Thinking selection -> use the latest assistant text -> use event-kind statistics. Claude summaries may select a Gateway profile; Codex summaries may select a `model_provider`; Grok keeps its model-alias routing. Grok uses the official headless CLI in a temporary strict-sandbox cwd with one turn, no memory/subagents/web search, no executable or MCP tools, bounded output, and exact ephemeral-session cleanup. The LLM writes a one-sentence description of "what the session is currently doing", shown on the second line of cards and in the SessionDetail Summary tab.
...
- Claude Code resources are in `resources/claude-config/`; Codex resources are in `resources/codex-config/`; Grok Build resources are in `resources/grok-config/`.
...
- Packaged assets remain immutable. In the Assets Library, bundled Agents may override model and Thinking; Claude and Codex Agents may additionally select or enter a Gateway profile / native `model_provider`. Bundled Skills remain read-only, while user/project Agents retain their existing full native editing behavior.
- Runtime overrides are stored as app-owned deltas keyed by `adapter:name`. **Restore default** deletes the complete delta and immediately reveals the packaged values again; it never copies or rewrites the packaged Agent file. Spawn precedence is explicit `spawn_session` provider/model/thinking input → Agent runtime/frontmatter → persisted same-adapter session selection → application default → adapter-native default.
- Every adapter still owns its endpoint, credentials, model aliases, and provider definitions: Claude Code uses `~/.claude/gateways/*.json`, Codex uses `~/.codex/config.toml` (including `[model_providers.<id>]`), and Grok uses `~/.grok/config.toml` (including `[model.<alias>]`). Agent Deck stores only profile ids and settings paths, passes the selection to the target child/thread, and never stores Gateway tokens in its database or settings.
...
Agent Deck does not store Claude Gateway, Codex, or Grok credentials in its database or application settings.
...
- **Claude Gateway profiles**: add one JSON settings file per profile under `~/.claude/gateways/`, for example `~/.claude/gateways/deepseek.json` or `openrouter.json`. The filename stem is the provider id shown in Agent Deck. Each Claude SDK child receives only its selected file through `options.settings`; Agent Deck never changes global `process.env`. On upgrade, if the Deepseek Gateway file is absent and the former `~/.agent-deck/.deepseek/settings.json` exists, startup copies it once to `~/.claude/gateways/deepseek.json` with restricted permissions; runtime code reads only the new Gateway path afterward.
- **Codex CLI**: run `codex auth` in a terminal first; the app directly reuses `~/.codex` config.
- **Grok Build**: install the official `grok` CLI, then either run `grok login --oauth` or configure a model `env_key` / `XAI_API_KEY` in the normal Grok environment. Agent Deck uses the configured binary path or resolves `grok` from the user shell `PATH`; the Settings → Grok Build page can run initialize/authenticate diagnostics without creating a session or sending a model prompt. Agent Deck never stores the key and never writes `~/.grok/config.toml`, `~/.grok/AGENTS.md`, or user plugins.
...
Selecting a custom model or provider on a bundled Agent does not configure its backend. Define the
Gateway settings file or native provider first. For Claude, `provider` selects
`~/.claude/gateways/<provider>.json`; for Codex it becomes the per-thread `model_provider`, while
the matching `[model_providers.<id>]` table remains in `~/.codex/config.toml`. Grok resolves the
entered model name through its native alias routing.
...
agent-deck new \
  ...
  [--agent claude-code|codex-cli|grok-build]  # short names claude|codex|grok are supported; --adapter is equivalent
  [--provider <runtime-provider>]          # Claude Gateway profile or Codex model_provider; rejected for Grok
...
The Settings panel opens from the gear button. It has four top-level tabs: **General** / **Claude Code** / **Codex CLI** / **Grok Build**. The General tab has four themed groups (**Sessions** / **Alerts And Appearance** / **Integration And Runtime** / **Cross-Tool Collaboration (MCP)**), and each group has sections whose titles can be collapsed / expanded (state is persisted in localStorage; only Lifecycle is expanded by default):
...
- **Sessions**
  ...
  - **Intermittent summaries**: used by session cards and the Summary view, never for hand-off or missing-history recovery. Controls include an enable switch (on by default), trigger interval / event count (default 30 events), concurrent summary limit (default 2; range 1–10), adapter, runtime provider/Gateway, model, and Thinking level. Claude Gateway profiles and Codex `model_provider` values use discovered dropdowns; Grok exposes no provider field. Each run freezes an event-revision boundary and builds a bounded read-only evidence snapshot; the card keeps a one-line headline while Summary history can show progress, next step, and risks.
  - **Continuation Context**: used by hand-off to a new session and missing-native-history recovery. Controls include automatic checkpoint maintenance (on by default), normal refresh interval (default 30 minutes; range 5–1,440), concurrent background-session limit (default 2; range 1–10), adapter / runtime provider / model / Thinking, and a recent-user-input ceiling (default 64,000 tokens; range 8,000–128,000). Its runtime selection is independent from intermittent summaries.
    ...
    - **Failure boundary**: Codex and Grok compact generation use hardened-but-unattested isolated oneshots with no executable or MCP tools; Codex additionally disables network through its app-server boundary, while Grok disables web search and runs in a temporary strict sandbox. Output is accepted only after schema canonicalization, exact evidence allowlisting, active-fact carry-forward checks, bounded fitting, and revision-safe persistence. UI and MCP hand-off share a 300-second checkpoint deadline; missing-history recovery keeps its independent 30-second limit because it operates on an already frozen spool and must return quickly. Provider or validation failure falls back to the last valid checkpoint plus immutable raw history.
...
- **Integration And Runtime**
  ...
  - **External tools**: Codex / Claude binary paths (leave empty to use the app's vendored versions) and the Grok Build binary path (leave empty to resolve `grok` from the user shell `PATH`; Grok is not bundled)
...
- **Cross-Tool Collaboration (MCP)**
  - **Agent Deck MCP server (on by default)**: after enabling it, authenticated Claude / Codex / Grok Build callers can orchestrate other coding-agent sessions across adapters, present plans and diffs to users for confirmation or feedback, manage structured tasks, and report issues through 19 tools (7 session / messaging: `spawn_session` / `send_message` / `list_sessions` / `get_session` / `list_session_events` / `shutdown_session` / `hand_off_session`; 2 user presentation: `present_plan` / `present_diff`; 2 worktree: `enter_worktree` / `exit_worktree`; 5 task: `task_create` / `task_list` / `task_get` / `task_update` / `task_delete`; 3 issue: `report_issue` / `append_issue_context` / `update_issue_status`). The runtime profile filters tool visibility and schema variants from the authenticated caller adapter; Grok Build, for example, receives fresh-only `contextMode` because its current ACP surface does not provide native fork. Third-party MCP clients can connect through the external transport, with available tools limited by the external-caller restrictions below. `spawn_session.provider` and `hand_off_session.provider` select a Claude Gateway profile or Codex `model_provider`; `spawn_session.model` and `hand_off_session.model` accept maintained suggestions grouped by adapter: Claude (`haiku`, `sonnet`, `opus`, `fable`, `deepseek-v4-flash`, `deepseek-v4-pro[1m]`), Codex (`gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.5`, `gpt-5.4`), and Grok Build (`grok-4.5`). Suggestions are not an allowlist: any non-empty provider model id is passed through unchanged for provider validation. Transport:
    Worktree lifecycle: unless a user or project requires a custom layout, callers omit `worktreePath` and `worktreeRoot`, and `enter_worktree` uses `<main-repo>/.agent-deck/worktrees`; the main repository `.gitignore` must contain `.agent-deck/`. Normal completion commits all intended changes and pushes the work branch before `exit_worktree` removes the worktree directory and marker. The local work branch is kept by default. An agent must ask the user immediately before every `deleteBranch: true` call and receive explicit approval; generic completion or cleanup instructions never authorize branch deletion.
    `spawn_session` and `hand_off_session` provider / model / thinking overrides apply only to the target session and never rewrite global defaults. Spawn precedence is explicit argument > resolved Agent runtime/frontmatter > persisted same-adapter source selection > application default > adapter-native default. A same-adapter hand-off inherits omitted provider / model / thinking values from the caller; a cross-adapter hand-off uses target defaults unless explicitly overridden. Codex thinking accepts `low` / `medium` / `high` / `xhigh` / `max` / `ultra`; Claude accepts `low` / `medium` / `high` / `xhigh` / `max`; Grok Build accepts `low` / `medium` / `high` / `xhigh`. Grok `sessionMode` accepts `default` / `plan` / `ask` and is rejected for other adapters. Creation failures preserve the underlying error and add an actionable `hint`; retry with its exact value or action, or omit the rejected optional override.
    `spawn_session.contextMode` accepts `fresh` or `fork` and defaults to `fresh`. A requested fork can use only an authenticated active caller whose runtime profile advertises native fork, requires the exact caller adapter, exact persisted runtime provider, and a target cwd resolving to the same real directory, and never accepts a source session id or turn count. Grok Build callers receive a fresh-only schema. A supported fork clones native provider history through the safe active-turn boundary: prior history and the current user request are included, while unfinished assistant reasoning/output, tool activity, and the active `spawn_session` frame are excluded. Successful fork results add `contextMode: "fork"` and the Agent Deck `forkedFromSessionId`; provider-native ids are not returned. If a fork fails, follow its exact hint or use `fresh`; there is no silent downgrade and no Gateway / `model_provider` switch. When a first-turn Codex caller has no terminal prefix, Agent Deck creates an independent zero-prefix target thread and replays the current native `UserInput` values before the delegated prompt. Native forks can carry large history, so use `fresh` when inherited context is unnecessary. `hand_off_session` always remains fresh: its `prompt` is the authoritative continuation instruction, while Agent Deck privately prepends the same versioned Continuation Context used by UI hand-off and missing-history recovery. Its result contains compact checkpoint/revision/token metadata rather than the provider prompt. The full persisted history stays with the source session. Before source close, tasks / active teams / the worktree marker / in-flight message endpoints move to the successor; existing issue source/resolution authority, pending plan gates, and related-session visibility follow the committed ownership chain without rewriting provenance.
    - **in-process**: mounted automatically for claude SDK sessions
    - **HTTP** `/mcp`: injected into Codex app-server and Grok ACP session config with a per-session Bearer token; external MCP clients can also connect
...
Settings has General, Claude Code, Codex CLI, and Grok Build views. Claude, Codex, and Grok expose user-scope terminal Hook controls; Grok also exposes native config guidance and an ACP authentication probe. The Grok Hook writes only `~/.grok/hooks/agent-deck.json`, while ACP authentication and model routing remain owned by the Grok CLI. The bottom of General has **Reset to default configuration**; it restores configurable preferences across all four views while preserving installation authentication tokens and installed terminal Hooks.
...
The **Asset Library** button on the right side of the Header toolbar opens an independent dialog that shows bundled Agent Deck assets plus user-defined `~/.claude/{agents,skills}/` and `~/.codex/{agents,skills}/` assets. Skills and Agents include a Grok sub-tab for its bundled read-only plugin assets. App conventions has Claude, Codex, and Grok editors; each saves an app-owned custom copy and shows **Restore default** only when that copy exists. The Grok copy lives at `<userData>/agent-deck-grok-agents.md`, is injected through ACP for new sessions, and never edits user-level `~/.grok` files. Injection switches control only bundled Agent Deck resources; user / project agents and skills are unaffected. Bundled resources are Agent Deck's behavior baseline; user-defined agents/skills are an enhancement layer and cannot replace bundled protocol or reviewer discipline. Claude/Codex user agents and skills support creating / editing / deleting user copies; saved copies are visible to the next new session. The only bundled Claude reviewer is `reviewer-claude`; a Deepseek review is requested explicitly by selecting the `deepseek` Gateway and a model for a normal Claude session.
...
│   ├── adapters/
│   │   ├── claude-code/   hook routes + hook installer + SDK bridge + Gateway profile discovery + CLAUDE.md / skill / agents injection + sandbox-config
│   │   ├── codex-cli/     codex app-server JSON-RPC bridge + hook routes/installer for external terminal sessions
│   │   └── grok-build/    ACP v1 bridge + native external Hook installer/routes + isolated headless generator
...
├── codex-config/          Codex counterpart assets: CODEX_AGENTS.md + agent-deck-plugin
└── grok-config/           Grok ACP baseline + plugin assets (see resources/README.md)
...
# Grok native Hook payloads use camelCase fields and adapter-specific routes
curl -sS -X POST http://127.0.0.1:47821/hook/grok/sessionstart \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer <token>" \
  -d '{"sessionId":"grok-test-001","cwd":"<any-directory>","hookEventName":"SessionStart"}'
```
