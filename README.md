...
- **Cross-Tool Collaboration (MCP)**
  - **Agent Deck MCP server (on by default)**: after enabling it, claude / deepseek / codex / third-party MCP clients can orchestrate other coding-agent sessions across adapters, request user plan reviews, manage structured tasks, and report issues through 17 tools (7 session / interaction: `spawn_session` / `send_message` / `request_plan_review` / `list_sessions` / `get_session` / `shutdown_session` / `hand_off_session`; 2 worktree: `enter_worktree` / `exit_worktree`; 5 task: `task_create` / `task_list` / `task_get` / `task_update` / `task_delete`; 3 issue: `report_issue` / `append_issue_context` / `update_issue_status`). Transport:
    ...
    Recursion protection: max spawn-chain depth (default 3) / per-minute spawn limit (default 20) / max child sessions per caller (default 10) / cwd realpath cycle detection across the entire chain. Message rate limit (default 60/min): team messages use per-team buckets, and teamless DMs use per-sender buckets shared across all receivers for the same sender. Replies no longer poll-wait; after `send_message` delivers, universal-message-watcher automatically injects into the target session and reply chain. Task tools automatically close over `owner_session_id = caller_session_id`; write permissions are shared with active team members; `hand_off_session` automatically transfers tasks with the baton. Issue records store cwd and best-effort git branch snapshots for triage context. The settings UI exposes all Agent Deck MCP server thresholds. Protocol details are governed by this README, bundled CLAUDE.md / CODEX_AGENTS.md, and MCP tool descriptions.

The Claude Code tab installs / uninstalls the hook into `~/.claude/settings.json` with one click (user scope). The Codex CLI tab installs / uninstalls the Codex hook into `~/.codex/hooks.json` with one click (user scope). Edit Codex's own config (model / sandbox / approval / external MCP servers) directly in `~/.codex/config.toml`; Agent Deck injects its app-server MCP section dynamically for in-app Codex sessions instead of persisting it to disk. Agent Deck's Codex app conventions are loaded automatically into newly created in-app Codex sessions and are not written into the user's `~/.codex/AGENTS.md`.
