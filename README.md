...
- **活动流 + Diff + 总结**：每条会话点开看消息时间线、按文件分组的 Monaco DiffEditor、阶段性 LLM 一句话总结；工具调用点 start 行展开完整入参、点 end 行展开出参，Task / Agent 工具调用专门渲染（subagent 名 + 紫色 chip + prompt 全文折叠/展开）
- **控制权交接提醒**：waiting → 红闪烁 + 提示音 + 系统通知 + Dock 弹跳；finished → 黄 + 完成音；可逐项关闭，可换自定义提示音
- **人机交互内嵌响应**（仅 SDK 会话）：工具权限请求、Claude 主动询问（选项 / 其他 / 备注）、Claude Plan mode 执行计划批准、MCP `request_plan_review` 计划检阅 —— 全部在活动流卡片里直接处理。批准 Claude Plan mode 时可选目标权限模式（默认 / 自动接受编辑 / 保持 Plan / 完全免询问）；切到「完全免询问」会自动重启 SDK 子进程
...
- **模型 Token 统计**：顶栏中部实时显示今日使用最频 Top3 模型的「输出 token/s」（最近 60 秒滑动窗口；窗口宽度不足时自动退化隐藏，先减到 Top1 再隐藏）。「数据」tab（与 实时/待处理/历史/团队/问题 同级）看每个模型每天的 token 使用：模型×日期表格（输入 / 输出 / 缓存读 / 缓存写）+ 今日汇总 + 全模型实时 token/s 区，并显示 Claude / Codex 账号的当前额度窗口、周用量和重置时间；应用启动时预取本地 token 明细并预热 provider 额度缓存，数据 tab 打开时复用缓存并定时刷新。优先复用已打开的 Claude / Codex 会话；没有 live 会话时会启动独立的静默额度探针读取 provider 额度，探针使用应用私有目录、不加载 Claude hook settings、不创建 Agent Deck 会话、不发用户消息或模型 turn，若 provider 要求交互授权则失败返回；Deepseek 因走 API 通道暂显示不支持。token 用量从每条 assistant message（Claude）/ turn 完成（Codex）采集；同基础模型的不同变体（thinking / 1m 等）按友好名合并统计；未显式指定模型时按 Claude / Codex 默认模型占位显示
...
| 状态 | 何时进入 |
|---|---|
| `active` | 默认；最近 active 窗口（默认 60 min）内有事件 |
...
`DiffRegistry` + `DiffRendererPlugin` 接口。内置 text（有 before/after 快照时用 Monaco DiffEditor；Codex 或旧记录仅提供 unified diff 时会先还原成 before/after 片段复用同一 Monaco DiffEditor，解析不了的 binary/rename patch 才显示原文；改动页的「最终 diff」基于会话记录时捕获的文件 before/after 快照，不再读取当前 Git 工作区，历史记录缺少快照时退回已记录 patch）/ image（侧并排 / after-only / 滑动对比三视图）/ pdf（占位）。新增渲染器在 `src/renderer/components/diff/install.ts` 注册即可。
...
调度器隔几分钟扫一次 active+dormant 会话，按时间或事件数触发，三层降级：按设置选择的 provider 跑一次低成本 LLM oneshot → 最近一条 assistant 文字 → 事件 kind 统计。LLM 一句话描述「会话当前在做什么」，显示在卡片第二行 + SessionDetail「总结」tab。

### 内置提示词资产

应用内 SDK 会话会加载 Agent Deck 打包的应用约定、reviewer agents 和 review skills；这些内置资源是运行时 baseline，用户自定义 agents/skills 只能增强，不能替代跨会话协议、reviewer 纪律或 MCP 边界。

- Claude Code / Deepseek 侧资源在 `resources/claude-config/`；Codex 侧资源在 `resources/codex-config/`。
- 同名 skill、reviewer agent 和应用环境约定按 Claude/Codex 对偶维护。协议语义必须一致，工具名、turn boundary、sandbox 细节按 adapter 写。
- 资源路径、打包位置和注入方式以 [resources/README.md](resources/README.md) 为准。
...
| **Windows 10 1703+ / 11** | **beta** | NSIS 安装包 + portable 双 target；wrapper 走 `agent-deck.cmd`；vibrancy 缺失（`titleBarStyle: hidden + frame: false` Win 也支持，但无毛玻璃）；Dock bounce 不存在（靠系统通知 + 声音提示）。Win 端真机 E2E 留待 CI 验证 |
| **Linux** | dev only | `pnpm dev` 能跑；`dist:linux` 配了 AppImage target 但未单独验证；`paplay → aplay` 音效兜底链路在无 PulseAudio 的桌面会脆弱 |

### macOS：装 dmg

```bash
# 0. 关闭旧实例（覆盖重装必做；明确要求不 kill 时只执行打包步骤）
...
agent-deck new \
  ...
  [--agent claude-code|deepseek-claude-code|codex-cli]  # 短名可用 --agent claude|deepseek|codex；--adapter 等价
  [--permission-mode default|acceptEdits|plan|bypassPermissions]  # wrapper 缺省补 bypassPermissions
  [--codex-sandbox workspace-write|read-only|danger-full-access]  # 仅 codex-cli 起效
  [--resume <sessionId>]                  # 续历史 jsonl
  [--team <name>] [--member <slug>:<adapter> ...]  # 起 team：lead + 若干 teammate（--member 可重复，须配 --team）
...
⚙ 按钮打开设置面板。顶部三个 tab：**通用** / **Claude Code** / **Codex CLI**。「通用」tab 按主题分 4 组（**会话** / **提醒与外观** / **集成与运行环境** / **跨工具协作（MCP）**），每组若干 section 可点标题折叠 / 展开（状态 localStorage 持久化；默认仅「生命周期」展开）：
...
│   ├── ipc/               所有 IPC handler 集中点（按域拆文件）；设置变更的运行时分发在 settings.ts
...
│   ├── utils/
│   │   └── logger.ts      electron-log v5 main 端封装（按天拆 + 14 天 cleanup + scoped logger + fatal hook + console 接管）
...
├── renderer/              React 19
│   ├── App.tsx            header（标题 / 统计 / pending chip / ＋ / 六个 tab：实时 / 待处理 / 历史 / 团队 / 问题 / 数据 / pin / 折叠 / 📚 / ⚙）
│   ├── components/        FloatingFrame · SessionList · SessionCard · SessionDetail ·
│   │                      PendingTab · pending-rows · PermissionsView · HistoryPanel ·
│   │                      NewSessionDialog · SettingsDialog (拆 settings/sections/*) ·
│   │                      AssetsLibraryDialog · assets/AssetEditor ·
│   │                      activity-feed (Task 渲染) · diff/ ·
│   │                      IssuesPanel · IssueDetail · DataPanel (token 统计) ·
│   │                      TeamHub · TeamDetail (Universal Team Backend 视图，走 agent-deck-team:* IPC + universal-message-watcher 投递)
│   ├── stores/            Zustand session store
│   ├── hooks/             事件桥接
│   ├── utils/
│   │   └── logger.ts      electron-log v5 renderer 端封装（IPC bridge 转发到 main → 同一份 main-YYYY-MM-DD.log）
...
├── claude-config/         应用打包内置的 CLAUDE.md + agent-deck-plugin（agents/skills），extraResources 复制到 .app
└── codex-config/          Codex 对偶资产：CODEX_AGENTS.md + agent-deck-plugin（详见 resources/README.md）
...
```bash
pnpm typecheck       # 必跑
pnpm test            # vitest（经 scripts/test-electron.mjs 用 Electron 内置 node 跑，better-sqlite3 单测真跑；文件顶部 `// @vitest-environment happy-dom` 切 React hook 测试）
pnpm test:node       # 系统 node 跑 vitest（SQLite 单测优雅 skip，非 SQLite 快速迭代用）
...
Schema 版本由 `user_version` pragma 增量推进（migration 文件在 `src/main/store/migrations/`，当前版本以该目录最新文件为准）。表：`sessions / events / file_changes / summaries / app_meta / tasks / agent_deck_teams / agent_deck_team_members / agent_deck_messages / issues / issue_appendices / token_usage`。
...
- `package.json > build.mac.icon = "resources/icon.png"` 必须显式指定
