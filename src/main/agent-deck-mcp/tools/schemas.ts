...
export const ARCHIVE_PLAN_SHAPE = {
  ...
  worktreePath: z
    ...
    .describe(
      'Absolute path to the plan worktree (e.g. /workspace/example/Repository/foo/.claude/worktrees/<planId>). Caller (mcp tool) must have already ExitWorktree-d before calling — handler refuses if the caller session cwd (or its held cwd_release_marker) is inside this path. Use ExitWorktree(action:"keep"), not a shell `cd`.',
    ),
  baseBranch: z
    .string()
    .min(1)
