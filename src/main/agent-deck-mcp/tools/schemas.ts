...
export const ARCHIVE_PLAN_SHAPE = {
  ...
  worktreePath: z
    ...
    .describe(
      'Absolute path to the plan worktree (e.g. /workspace/example/Repository/foo/.claude/worktrees/<planId>). Caller (mcp tool) must have already ExitWorktree-d before calling — handler refuses if process.cwd() is inside this path.',
    ),
  baseBranch: z
    .string()
    .min(1)
