...
export const ARCHIVE_PLAN_SHAPE = {
  ...
  worktreePath: z
    ...
    .describe(
      'Absolute path to the plan worktree (e.g. /workspace/example/Repository/foo/.claude/worktrees/<planId>). Before calling archive_plan, leave this worktree with exit_worktree({ action: "keep" }) or Claude ExitWorktree(keep); shell `cd` is not enough because the caller session may still hold a worktree marker.',
    ),
  baseBranch: z
    .string()
    .min(1)
