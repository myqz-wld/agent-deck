...
export const ARCHIVE_PLAN_SHAPE = {
  ...
  worktree_path: z
    ...
    .describe(
      'Absolute path to the plan worktree (e.g. /workspace/example/Repository/foo/.claude/worktrees/<plan_id>). Caller (mcp tool) must have already ExitWorktree-d before calling — handler refuses if process.cwd() is inside this path.',
    ),
  base_branch: z
    .string()
    .min(1)
