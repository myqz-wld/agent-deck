...
export async function buildAgentDeckTools(
  ...
  const spawnSession = tool(
    ...
    'Spawn a parallel SDK session on claude-code, deepseek-claude-code, or codex-cli. Required fields: adapter, absolute cwd, and a complete non-empty prompt. Omit agentName for a general-purpose teammate; set it only to resolve an adapter-native bundled, project, or user agent. Optional model and thinking are target-session-only overrides; their field schemas list maintained suggestions, adapter-specific values, precedence, and custom-model passthrough. Pass teamName to create or reuse a shared team; omit it for a standalone session that can still use teamless DM. Returns sessionId, optional teamId, spawnPromptMessageId, and spawnLimits. On failure, follow hint exactly or omit the rejected optional override instead of guessing. Use hand_off_session when replacing the current session.',
    SPAWN_SESSION_SCHEMA,
