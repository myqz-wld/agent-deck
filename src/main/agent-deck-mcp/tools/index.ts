...
export async function buildAgentDeckTools(
  ...
  const enterWorktree = Object.assign(tool(
    ...
    'Create a fresh git worktree and automatically move this in-app session to it at a safe provider turn boundary. `baseBranch` is required and must resolve to `refs/heads/<baseBranch>`; SHA, tag, remote-only refs, and rev syntax are rejected. Omit `workBranch` for a unique generated branch. Unless the user or project requires another layout, omit `worktreePath` and `worktreeRoot` to use `<main-repo>/.agent-deck/worktrees`; first ensure the main repository `.gitignore` contains the exact `.agent-deck/` entry. The authenticated caller must be a live Claude, Codex, or Grok session inside that Git repository, and the handler must claim exactly one provider-observed enter_worktree invocation; missing or ambiguous tool identity fails before creation. The first accepted call creates the branch/worktree, persists a generation-scoped transition plus legacy marker, and seals post-result ingress. Success state `waiting-tool-result` is asynchronous acceptance, not proof that the current turn already runs in the new cwd: after this exact result reaches the provider, Agent Deck fences later old-turn work, issues an expected interrupt, applies the worktree cwd to runtime and database, queues one fixed internal continuation, then replays buffered user inputs in FIFO order. `effectiveFrom:"automatic-next-turn"` names that boundary. Do not issue `cd` or start another tool to perform the switch. A repeat while the same enter is still creating/waiting returns its existing transition; nested enter after activation and conflicting generations are rejected with a recovery hint. Runtime, persistence, or restart failures retain or safely roll back the structured lease instead of guessing cwd. Success returns identical JSON text and structuredContent matching the published output schema; errors do not.',
    ENTER_WORKTREE_SCHEMA,
  ...
  const exitWorktree = Object.assign(tool(
    ...
    'Automatically restore this in-app session to its original cwd, then safely remove its owned worktree. For normal completion, commit all intended changes and successfully push the work branch first; if commit or push fails, retain the worktree and lease. Omit `worktreePath` to use the structured lease or legacy marker; an override must match exactly. Existing marker-only or explicitly named registered worktrees are first checked with bounded Git commands and adopted into the same structured restore-first flow; detached worktrees return `workBranch:null`. An accepted exit requires one exact provider-observed exit_worktree invocation and returns `waiting-tool-result`, never inline removal. After this exact result reaches the provider, Agent Deck fences the old turn, issues an expected interrupt, restores and confirms runtime/database cwd, checks all persisted/runtime/lease references, performs a second dirty check immediately before removal, queues one fixed internal continuation, and replays buffered user inputs in FIFO order. Do not issue `cd` or continue old-cwd tool work. If removal cannot finish after cwd restoration, the session remains safely in the original cwd with state `cleanup_pending`; retry exit_worktree to finish cleanup. Retry success returns `completed-cleanup`; `completed-legacy` is synchronous only when the target path is already absent. `discardChanges` defaults false and requires explicit user authorization to abandon dirty changes. `deleteBranch` defaults false and requires explicit approval immediately before every true call; generic finish/cleanup wording or pushed, merged, cherry-picked, or abandoned branch state is not approval. Worktree deletion never occurs while any runtime or lease still references it. Success returns identical JSON text and structuredContent matching the published output schema; errors do not and may include `markerCleared` for recovery.',
    EXIT_WORKTREE_SCHEMA,
