...
export async function buildAgentDeckTools(
  ...
  const spawnSession = tool(
    ...
    'Spawn a parallel SDK session on "claude-code", "deepseek-claude-code", or "codex-cli". Omit `agentName` for a normal/general-purpose teammate; pass `agentName` only when you intentionally need a real adapter-native agent. Resolution is adapter-scoped: bundled Agent Deck reviewers, then project agents under .claude/agents or .codex/agents, then user agents under ~/.claude/agents or ~/.codex/agents. Put the complete task in `prompt`. Optionally pass `model` (a maintained alias or custom provider model id) and adapter-scoped `thinking` values from the tool schema. Pass `teamName` to create or reuse a shared team where the caller is lead; omit it for a standalone session that can still reply by teamless DM. Returns the new `sessionId`, optional `teamId`, `spawnPromptMessageId` for the first reply chain, and `spawnLimits` with depth/fan-out/rate state; guard denials report the violated limit in `spawnLimits`. Use `hand_off_session` instead when the current session should be replaced.',
    SPAWN_SESSION_SCHEMA,
