...
export async function buildAgentDeckTools(
  ...
  const handOffSession = tool(
    ...
    'Hand off the current SDK session to a fresh successor when this session should be replaced, such as a context reset or the next work phase. Put the explicit continuation instruction in `prompt`; Agent Deck automatically builds a versioned compact capsule from the latest stored checkpoint, a bounded tail of recent raw user/assistant messages, and that final instruction. Historical sections are labeled as evidence, while the source session keeps its complete persisted history. Omit adapter to inherit the caller adapter, or choose claude-code, deepseek-claude-code, or codex-cli; model is optional free text and thinking is an adapter-aware enum. Omitted model/thinking inherit on same-adapter hand-offs and use target defaults across adapters. The tool transfers caller-owned tasks, active team memberships, and the worktree marker, then closes the caller only after mandatory transfer succeeds; on transfer failure it returns an MCP error and the caller stays open. Returns successor spawn fields, compact-context metadata, and transfer details. Use spawn_session for parallel work.',
    HAND_OFF_SESSION_SHAPE,
    async (args, extra) => {
