...
export function executionCutoverError(
  ...
  if (reason === 'target-startup-timeout') {
    if (successorCleanup === 'ok') {
      return {
        ...
        hint:
          'No successor remains and no resources moved. Prepare a fresh continuation context before retrying.',
      };
    }
    return {
      error: 'handoff successor startup exceeded the trusted continuation readiness deadline',
      hint:
        'No resources moved and no stable successor id was available. Late-candidate cleanup is pending and will be attempted best-effort; inspect the session list and application logs before retrying.',
    };
  }
  ...
  const postAcceptancePrefix =
    `${cleanupPrefix} ` +
    'The successor may have partially executed the continuation before it was aborted; retrying can duplicate tool, command, file, or external effects.';
  const mayHavePartiallyExecuted = handOffSuccessorMayHavePartiallyExecuted({
    successorSessionId,
    ...(reason ? { cutoverReason: reason } : {}),
  });
  const effectAwarePrefix = mayHavePartiallyExecuted ? postAcceptancePrefix : cleanupPrefix;
  if (reason === 'late-message-delivery-failed') {
    return {
      error: 'failed to deliver late source messages to the handoff successor',
      hint:
        `${effectAwarePrefix} The source remains active; retry after the target adapter can accept the queued messages.`,
    };
  }
  if (reason === 'message-delivery-drain-timeout') {
    return {
      ...
      hint:
        `${effectAwarePrefix} The source remains active; retry after its active cross-session delivery reaches a durable terminal or retry state.`,
    };
  }
  if (reason?.startsWith('target-')) {
    return {
      error: mayHavePartiallyExecuted
        ? 'handoff successor failed trusted continuation readiness'
        : 'handoff successor did not cross the trusted continuation readiness boundary',
      hint:
        `${effectAwarePrefix} ` +
        'The source remains active; inspect the target provider and retry when appropriate.',
    };
  }
  return {
    error: 'source session changed while creating the handoff successor',
    hint: `${effectAwarePrefix} Prepare a fresh continuation context and retry.`,
  };
}
