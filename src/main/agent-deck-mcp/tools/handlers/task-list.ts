 ...
 * - **不传 team_id_filter**:走 visibleScope OR 模式 — caller-owned personal ∪ caller 所在
 *   active team 的 team task 一次 SQL 完成
 * - **传具体 team_id**:走 teamIdFilter=team_id（先校验 caller 在该 team 是 active member,
 *   否则 reject）
 * - **传字面量 'null-personal'**:走 teamIdFilter='null-personal' + ownerSessionIds=[callerSid],
 *   仅返 caller 自己 personal task（owner == caller AND team_id IS NULL）
 *
...
export const taskListHandler = withMcpGuard(
  ...
  async (args: TaskListArgs, ctx: HandlerContext) => {
    try {
      ...
      let tasks;
      if (args.team_id_filter === undefined) {
      ...
      } else if (args.team_id_filter === 'null-personal') {
        // 仅 caller 自己 personal task（owner == caller AND team_id IS NULL）
        tasks = taskRepo.list({
          status: args.status_filter,
          subjectKeyword: args.subject_filter,
        ...
        // 具体 team_id:校验 caller 在该 team active member（D5 + §不变量 13 双条件）
        if (!isCallerInTeam(callerSid, args.team_id_filter)) {
          return err(
            `caller "${callerSid}" is not an active member of team_id "${args.team_id_filter}" — task_list rejected (v024 plan §D5)`,
            'team_id_filter 要求 caller 在该 team 是 active member（agent_deck_team_members.left_at IS NULL AND agent_deck_teams.archived_at IS NULL 双条件）。Use task_list without team_id_filter for caller visible scope, or join the team first via the application UI.',
          );
        }
        tasks = taskRepo.list({
          status: args.status_filter,
          subjectKeyword: args.subject_filter,
          teamIdFilter: args.team_id_filter,
          limit: effectiveLimit,
          offset: args.offset,
        });
      }
