 ...
 *
 * - 旧 REVIEW_46 B-HIGH-2: `archiveCaller=false` 退化 normal spawn 走完整 spawn-guards;
 *   plan §D4 推翻 — `archiveCaller=false` 也走 hand-off 路径(handOffMode=true)完全跳过
 *   三道防御,power-user 自负责任(§D3 + §D4)
 * - 旧 REVIEW_47 M12: `archiveCaller=false` 时 batonRole=undefined 让 spawn 走默认 'teammate';
 *   plan §D5 推翻 — hand-off 两路径都用 batonRole='lead'(无 archiveCaller 分流)
 *
 ...
 *   入参签名简化为无参 + 无 archiveCaller 分流)
 * - **handler 集成 test**:mock spawn handler 断言 `opts.handOffMode` / `opts.batonRole`
 *   真实传入值 = `{ handOffMode: true, batonRole: 'lead' }`,不论 archiveCaller 值
 */
...
describe('resolveBatonRoleForSpawn lambda (plan §D8 退化常量)', () => {
  ...
  // plan §D5 推翻 REVIEW_47 M12 修法语义验证:hand-off 两路径(archiveCaller true/false)都用
  // batonRole='lead',不再有 archiveCaller=false → batonRole=undefined 分流
  it('§D5 推翻 REVIEW_47 M12: 无 archiveCaller 分流 — lambda 不接受任何入参', () => {
...
describe('handOffSessionHandler — hand-off 路径完全跳过 spawn-guards / 永不写 spawn-link (§D1/§D4/§D5/§D6 端到端)', () => {
  ...
  // 推翻 REVIEW_46 B-HIGH-2:archiveCaller=false 不再退化 normal spawn,仍走 hand-off 路径
  it('archiveCaller=false → spawn opts.handOffMode === true + opts.batonRole === lead (§D1/§D4 推翻 B-HIGH-2)', async () => {
    ...
    // hand-off-mcp-archive-opt-20260515: archiveCaller=false 时 runBatonCleanup 跳 archive 段
    ...
    const args: HandOffSessionArgs = {
      planId: planId,
      adapter: 'claude-code',
      archiveCaller: false, // ← 推翻 B-HIGH-2:archiveCaller=false 不再退化 normal spawn
    };
  ...
  it('archiveCaller=true 显式 → spawn opts.handOffMode === true + opts.batonRole === lead', async () => {
    const state = makeState();
    ...
    const args: HandOffSessionArgs = {
      planId: planId,
      adapter: 'claude-code',
      archiveCaller: true, // ← 显式 true（与 undefined default 同语义）
    };
    const ctx: HandlerContext = {
      caller: { callerSessionId: 'caller-sid', transport: 'in-process' },
    };
  ...
  it('archiveCaller 不传 (default) → spawn opts.handOffMode === true + opts.batonRole === lead', async () => {
    const state = makeState();
    ...
    const args: HandOffSessionArgs = {
      planId: planId,
      adapter: 'claude-code',
      // archiveCaller 不传 — default 走 hand-off 路径
    };
    const ctx: HandlerContext = {
      caller: { callerSessionId: 'caller-sid', transport: 'in-process' },
    };
  ...
  // §D6 + §D7 守门:archiveCaller=false + teamName 显式组合下也走 hand-off 路径(不被 teamName 反转)
  it('archiveCaller=false + teamName 显式 → spawn opts 仍走 hand-off 路径 (§D6 守门)', async () => {
    const state = makeState();
    ...
    const args: HandOffSessionArgs = {
      ...
      adapter: 'claude-code',
      archiveCaller: false,
      teamName: 'foo-team', // ← caller 显式启用 team 通信
    };
    const ctx: HandlerContext = {
      caller: { callerSessionId: 'caller-sid', transport: 'in-process' },
    };
    ...
    // teamName 透传给 spawn args（独立于 handOffMode 决策）
    expect(spawnArgs.teamName).toBe('foo-team');

    // §D6 守门:archiveCaller=false + teamName 仍走 hand-off 路径,不被 teamName 反转
    expect(opts?.handOffMode).toBe(true);
    expect(opts?.batonRole).toBe('lead');
