...
import {
  ...
  type ClaudeForkCleanupObserver,
...
export interface ClaudeForkSdk extends ClaudeForkCleanupSdk {
  getSessionMessages(
    sessionId: string,
    options: { dir: string },
...
export interface ClaudeFamilyForkHost {
  loadSdk(): Promise<ClaudeForkSdk>;
  readConfigRoot(): string;
  childSessionStore: ClaudeForkChildStore;
  cleanupObserver: ClaudeForkCleanupObserver;
}
...
export async function createClaudeFamilyForkedSessionCore(
  args: CreateClaudeFamilyForkArgs,
  host: ClaudeFamilyForkHost,
): Promise<ForkedSessionHandle> {
  const sdk = args.sdk ?? (await host.loadSdk());
  const store = args.childSessionStore ?? host.childSessionStore;
  const configRoot = args.configRoot ?? host.readConfigRoot();
  ...
  const cleanup = createClaudeForkCleanupCore({
