...
export interface ClaudeCanUseToolHost {
  createRequestId(): string;
  now(): number;
  observeSandboxIntercept(host: string): void;
}
...
export function makeCanUseToolCore(
  ...
  return async (toolName, input, ctx) => {
    ...
    const requestId = host.createRequestId();
    ...
    if (toolName === 'SandboxNetworkAccess') {
      ...
      try {
        host.observeSandboxIntercept(interceptedHost);
      } catch {
        // Diagnostics cannot alter the permission decision.
      }
      return {
        behavior: 'deny',
        message:
          `网络访问被沙盒拦截（host: ${interceptedHost}）。如确实需要联网，请用 Bash + ` +
