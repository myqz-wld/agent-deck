...
export type RestartThunk = (
  sessionId: string,
  mode: PermissionMode,
  handoffPrompt: string,
) => Promise<string>;

export interface ClaudePermissionResponderHost {
  persistPermissionMode(sessionId: string, mode: PermissionMode): void;
  observeHotSwitchFailure(sessionId: string, error: unknown): void;
  observeColdSwitchFailure(sessionId: string, error: unknown): void;
  now(): number;
}

export class PermissionResponderCore {
  constructor(
    private readonly ctx: ResponderCtx,
    private readonly restartThunk: RestartThunk,
    private readonly host: ClaudePermissionResponderHost,
  ) {}
  ...
  async respondExitPlanMode(
    ...
    if (response.decision === 'approve') {
      ...
      try {
        ...
        this.host.persistPermissionMode(sessionId, response.targetMode);
      } catch (err) {
        s.permissionMode = oldMode; // 回滚 cache（与 index.ts setPermissionMode catch 同款）
        try {
          this.host.observeHotSwitchFailure(sessionId, err);
        } catch {
          // Diagnostics cannot suppress the user-visible rollback event.
        }
        this.ctx.emit({
          sessionId,
          agentId: AGENT_ID,
    ...
    if (response.decision === 'approve-bypass') {
      ...
      try {
        ...
        try {
          this.host.observeColdSwitchFailure(sessionId, err);
        } catch {
          // Restart failure is already surfaced by the restart controller.
        }
      }
    }
