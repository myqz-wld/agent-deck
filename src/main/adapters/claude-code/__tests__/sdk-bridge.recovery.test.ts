...
describe('sdk-bridge.sendMessage 断连自愈（B 方案）', () => {
  ...
  it('cwd 不存在时父目录 fallback 命中 → fallback main repo + 走 jsonl missing 同款下游', async () => {
    ...
    bridge.cwdExistsOverride = new Map<string, boolean>([
      ['/workspace/example/myrepo/.agent-deck/worktrees/dead-task', false],
      ['/workspace/example/myrepo', true],
    ]);
    vi.mocked(sessionRepo.get).mockReturnValue({
      id: 'sess-cwd-bad',
      agentId: 'claude-code',
      cwd: '/workspace/example/myrepo/.agent-deck/worktrees/dead-task',
      title: 'x',
      source: 'sdk',
      lifecycle: 'dormant',
      activity: 'idle',
    ...
    expect(bridge.recoveryCaptureCalls[0]).toMatchObject({
      emitCount: 0,
      overrides: { cwd: '/workspace/example/myrepo' },
    });
    ...
    expect((fallbackInfo[0]!.payload as { text: string }).text).toContain('/workspace/example/myrepo');
  ...
  it('CHANGELOG_99: cwd 不存在 + 启发式 1 不命中 + parent walk 命中 → fallback 到父目录', async () => {
    ...
    // 父目录 walk 找到第一个存在的目录
    bridge.cwdExistsOverride = new Map<string, boolean>([
      ['/workspace/example/some/deep/dead/cwd', false],
      ['/workspace/example/some/deep/dead', false], // parent 1
      ['/workspace/example/some/deep', false], // parent 2
      ['/workspace/example/some', true], // parent 3 命中 ✓
    ]);
    vi.mocked(sessionRepo.get).mockReturnValue({
      id: 'sess-walk',
      agentId: 'claude-code',
      cwd: '/workspace/example/some/deep/dead/cwd',
      title: 'x',
      source: 'sdk',
      lifecycle: 'dormant',
      activity: 'idle',
    ...
    expect(bridge.createCalls).toHaveLength(1);
    expect(bridge.createCalls[0]?.cwd).toBe('/workspace/example/some');
  ...
  it('cwdFellBack=true + 历史可用 → trusted continuation + cwd 成功提示', async () => {
    ...
    expect(bridge.createCalls).toHaveLength(1);
    expect(bridge.createCalls[0]?.cwd).toBe('/workspace/example/myrepo');
  ...
  it('parity-plan A.9: jsonl 不存在 + record extraAllowWrite=[mainRepo] → fallback 透传', async () => {
    ...
    vi.mocked(sessionRepo.get).mockReturnValue({
      ...
      extraAllowWrite: ['/workspace/example/mainrepo'],
    });
    ...
    expect(bridge.createCalls[0].extraAllowWrite).toEqual(['/workspace/example/mainrepo']);
  ...
  it('parity-plan A.9: 正常 resume + record extraAllowWrite=[mainRepo] → 透传', async () => {
    ...
    vi.mocked(sessionRepo.get).mockReturnValue({
      ...
      permissionMode: null,
      claudeCodeSandbox: 'workspace-write',
      extraAllowWrite: ['/workspace/example/mainrepo', '/workspace/example/anotherrepo'],
    });
    ...
    expect(bridge.createCalls[0].extraAllowWrite).toEqual([
      '/workspace/example/mainrepo',
      '/workspace/example/anotherrepo',
    ]);
  });
