...
export interface AdapterCapabilities {
   ...
   * SDK/ACP 通道有 internal session 的 adapter 都置 true。
   * 纯 hook-only adapter 置 false。
   */
  canCloseSession: boolean;
  /**
   * 是否支持作为 team member 接收 cross-adapter 消息（R3.E0 ADR §3.1 / E4 新增）。
   * - built-in SDK/ACP adapters: true（sendMessage 把外来文字塞进 user turn）
   *
   ...
   * - Claude/Codex: true；Grok Build 由 ACP initialize 动态协商。
   *
