...
describe('session model option normalization', () => {
  it('keeps model ids open-ended, trims them, and maps Deepseek aliases for creation', () => {
    expect(
      resolveCreateSessionModelOptions('deepseek-claude-code', {
        model: '  v4-pro ',
        thinking: 'max',
      }),
    ).toEqual({
...
describe('SessionModelController', () => {
  ...
  it('persists and applies a live next-turn selection', async () => {
    ...
    await controller.setOptions('session-1', { model: 'new-model', thinking: 'high' });

    expect(record).toMatchObject({ model: 'new-model', thinking: 'high' });
  ...
  it('rolls DB and live settings back when the provider rejects the change', async () => {
    ...
    await expect(
      controller.setOptions('session-1', { model: 'new-model', thinking: 'max' }),
