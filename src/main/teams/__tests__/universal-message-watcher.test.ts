...
vi.mock('@main/store/agent-deck-team-repo', () => ({
  agentDeckTeamRepo: makeAgentDeckTeamRepoMock({
    overrides: {
      ...
      }) as AgentDeckTeamRepo['findActiveMembershipIn'],
    },
  }),
}));
...
beforeEach(() => {
  ...
  teamRepoGetCalls.length = 0;
  membershipInCalls.length = 0;
...
describe('universal-message-watcher.deliver — plan teamless-dm-20260601 D5 teamless 短路', () => {
  it('teamless DM (teamId=null) 跳过 team 闸门（不调 agentDeckTeamRepo.get / findActiveMembershipIn）但仍 dispatch + markDelivered', async () => {
    const teamlessMsg = makeMessage({
      id: 'teamless-1',
      teamId: null, // teamless
      fromSessionId: 'sender-sid',
      toSessionId: 'receiver-sid',
      body: 'teamless dispatch',
    });
    nextClaimResult = { ...teamlessMsg, status: 'delivering' };
    // target + from session 都 active（session 级闸门保留，必须过）
    sessionByIdMap.set('receiver-sid', { id: 'receiver-sid', lifecycle: 'active', agentId: 'claude-code', archivedAt: null });
    sessionByIdMap.set('sender-sid', { id: 'sender-sid', lifecycle: 'active', agentId: 'claude-code', archivedAt: null });
    nextAdapterResult = {
      capabilities: { canCollaborate: true },
      receiveTeammateMessage: receiveTeammateMessageStub,
    };
    // 故意把 team repo 设成「若被调用会 fail」：teamless 必须**完全不碰** team repo。
    nextTeamResult = null; // 若 D5 短路失效会走 'team not found' markFailed
    nextMembershipResult = null; // 若被调会触发 membership fail

    const watcher = new UniversalMessageWatcher();
    await callDeliver(watcher, teamlessMsg);

    // 关键断言 1：team 闸门完全没被触碰（D5 短路）
    expect(teamRepoGetCalls).toHaveLength(0);
    expect(membershipInCalls).toHaveLength(0);
    // 关键断言 2：session 级闸门保留 + 投递成功
    expect(claimCalls).toEqual(['teamless-1']);
    expect(sessionRepoGetCalls).toContain('receiver-sid');
    expect(receiveTeammateMessageCalls).toHaveLength(1);
    expect(receiveTeammateMessageCalls[0]?.to).toBe('receiver-sid');
    expect(markDeliveredCalls).toHaveLength(1);
  ...
  it('teamless DM target archived → 仍 markFailed（session 级闸门对 teamless 也保留）', async () => {
    const teamlessMsg = makeMessage({
      id: 'teamless-archived',
      teamId: null,
      fromSessionId: 'sender-sid',
      toSessionId: 'receiver-sid',
      body: 'to archived',
    ...
    nextClaimResult = { ...teamlessMsg, status: 'delivering' };
    // target archived（session 级闸门，teamless 也必须拦）
    sessionByIdMap.set('receiver-sid', { id: 'receiver-sid', lifecycle: 'active', agentId: 'claude-code', archivedAt: Date.now() });
    sessionByIdMap.set('sender-sid', { id: 'sender-sid', lifecycle: 'active', agentId: 'claude-code', archivedAt: null });
    ...
    expect(markFailedCalls).toHaveLength(1);
    expect(markFailedCalls[0]?.reason).toMatch(/archived/);
...
describe('universal-message-watcher.process - REVIEW_35 HIGH-A1 backpressure 死锁修复', () => {
  ...
  async function runProcessWithPending(pending: AgentDeckMessage[], maxInflight = 10): Promise<{
    ...
    const watcher = new UniversalMessageWatcher();
    // REVIEW_100 R2: process() 入口加了 `if (!running) return` guard；白盒直调 process()
    // 的 test 需显式置 running=true 模拟「active watcher tick」（否则早退 0 deliver）。
    (watcher as unknown as { running: boolean }).running = true;
...
describe('universal-message-watcher.process — REVIEW_86 MED over-cap target 不被 under-cap target 流量饿死', () => {
  ...
  async function runOneTick(pending: Map<string, AgentDeckMessage>, maxInflight = 10): Promise<void> {
    ...
    const watcher = new UniversalMessageWatcher();
    // REVIEW_100 R2: process() 入口 `if (!running) return` guard → 白盒直调需置 running=true。
    (watcher as unknown as { running: boolean }).running = true;
    await (watcher as unknown as { process: () => Promise<void> }).process();
  }
...
describe('universal-message-watcher.stop — REVIEW_100 LOW: 停后不 reschedule', () => {
  ...
  it('stop 后 process() 入口 guard 早退（running=false → 不 claim/deliver）— REVIEW_100 R2', async () => {
    // codex R2 LOW: commit 15b0080 只拦 finally 再 schedule，拦不住**已 queued** 的
    // setImmediate(process) callback（stop 前排入 event loop，stop 后轮到执行仍进 process）。
    // 修法:process() 入口加 `if (!running) return`。本 test 模拟「callback 已 queued 后 stop」:
    // running=false（stop 后）下直调 process() 应入口早退,不碰 findEligible/claim。
    statefulPendingMap = new Map();
    // 放一条 pending：若入口 guard 失效，process() 会 claim+deliver 它（断言能抓住）
    statefulPendingMap.set('m1', makeMessage({ id: 'm1', toSessionId: 'recv-X', sentAt: 1 }));
    statefulMaxInflight = 10;
    nextSessionResult = { id: 'recv-X', lifecycle: 'active', agentId: 'claude-code' };
    nextAdapterResult = {
      capabilities: { canCollaborate: true },
      receiveTeammateMessage: receiveTeammateMessageStub,
    };
    ...
    const watcher = new UniversalMessageWatcher();
    const w = watcher as unknown as { running: boolean; process: () => Promise<void> };
    // running=false 模拟「stop 已调用，但 queued callback 仍轮到执行」
    w.running = false;
    await w.process();
    // 入口 guard 早退 → 不 claim 任何消息（修前 queued callback 会 claim+deliver m1）
    expect(claimCalls).toHaveLength(0);
    expect(statefulPendingMap.get('m1')!.status).toBe('pending'); // 未被 deliver
  });

  it('stop 后 finally 不 reschedule（running guard 拦 setImmediate）', async () => {
    ...
    // running=false → 入口早退 + finally 不 setImmediate → process 只被显式调的那 1 次
