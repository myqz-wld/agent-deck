...
describe('deriveTitle', () => {
  it('POSIX：取 basename', () => {
    expect(deriveTitle('/workspace/example/agent-deck')).toBe('agent-deck');
  });

  it('POSIX：去尾斜杠后取 basename', () => {
    expect(deriveTitle('/workspace/example/foo/')).toBe('foo');
  });
  ...
  it('Win：反斜杠路径（在 POSIX 平台 path.basename 用当前平台规则，不会切反斜杠）', () => {
    ...
    const result = deriveTitle('C:\\workspace\\example\\foo');
