/**
 * Event revision read model for the continuation-context pipeline.
 *
 * Revisions are allocated exclusively by the v037 SQLite triggers. This repository deliberately
 * performs no JSON parsing: a malformed historical payload must remain visible to the capture
 * layer instead of creating a silent revision-coverage hole.
 */
...
/**
 * DB-injected seam for focused repository tests and connection-local capture code.
 */
export function createEventRevisionRepo(db: Database): EventRevisionRepo {
...
function getDefaultRepo(): EventRevisionRepo {
  ...
  return createEventRevisionRepo(getDb());
}
