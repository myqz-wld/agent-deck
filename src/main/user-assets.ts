 ...
 * §D2 §D3 §D7 三 adapter user 自定义补齐）。
 *
 * 数据源：Claude/Codex 原生 root 与 Grok 原生 root/plugin scan
 *   - **claude-code**:
 ...
 *
 * Grok plugin-owned components are listed for visibility and deliberately remain read-only.
 */
import {
  existsSync,
  lstatSync,
 ...
 *
 * **plan §D7**：各 adapter root scan 合并：
 * - claude-code agents（~/.claude/agents/）
 * - claude-code skills（~/.claude/skills/）
 * - codex-cli agents（~/.codex/agents/*.toml）
 * - codex-cli skills（~/.codex/skills/）
 * - grok-build agents/skills（~/.grok/{agents,skills}/）及原生 user plugins
 *
...
export function getUserAssetContent(
  ...
  if (adapter === 'grok-build') {
    const grokPath = getGrokUserAssetPath(kind, name, pathHint);
    if (!grokPath) return { ok: false, reason: `not found: ${adapter}/${kind}/${name}` };
    try {
      return { ok: true, content: readFileSync(grokPath, 'utf8') };
    } catch (err) {
 ...
 * - grok-build agent → ~/.grok/agents/<name>.md
...
function resolveUserAssetPath(
  ...
  return getGrokUserWritePath(kind, name);
}
...
export function saveUserAsset(input: UserAssetInput): { ok: true } | { ok: false; reason: string } {
  ...
  if (!valid.ok) return { ok: false, reason: valid.reason };
  const validName = input.adapter === 'grok-build' ? isSafeGrokAssetName(input.name) : isSafeName(input.name);
  if (!validName) {
    return { ok: false, reason: `name 非法：${input.name}` };
  }
...
export function deleteUserAsset(
  ...
  if (adapter === 'grok-build') {
    const path = getGrokUserAssetPath(kind, name, pathHint);
    if (!path) return { ok: true };
    if (!isEditableGrokUserAssetPath(path, kind, name)) {
      return { ok: false, reason: 'Grok 原生插件资产由 Grok 管理，请使用 Grok 的插件命令修改' };
    }
    try {
      if (kind === 'agent') {
        const lst = lstatSync(path);
        if (lst.isSymbolicLink()) return { ok: false, reason: 'agent 是 symlink，请在文件管理器手动删除' };
        unlinkSync(path);
      } else {
        const dir = dirname(path);
        const lst = lstatSync(dir);
        if (lst.isSymbolicLink()) return { ok: false, reason: 'skill 目录是 symlink，请在文件管理器手动删除' };
...
function buildMarkdownAssetFileText(input: UserAssetInput, description: string, body: string): string {
  ...
  if (input.kind === 'agent') {
    ...
    if (input.provider !== undefined && input.provider.trim().length > 0 && input.adapter === 'claude-code') {
      fm.provider = input.provider.trim();
    }
    if (input.thinking !== undefined && input.thinking.trim().length > 0 && input.adapter === 'grok-build') {
      fm.effort = input.thinking.trim();
    }
  }
