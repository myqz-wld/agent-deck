 ...
 *
 * agent 上报问题机制（report_issue + append_issue_context 两个 mcp write tool）+ UI 顶层
 * Issues tab 看板的统一类型 SSOT。设计要点（详 plan §不变量 + §D 决策表）:
 *
 ...
 * - **status 严格 3 态**（§D7）：open（默认）/ in-progress / resolved；agent 永不修改，
 *   IPC IssuesUpdate args zod 严格 enum reject 其他值
 * - **kind 软枚举 + free-form fallback**（§D6）：5 个推荐值 + 任意其他 string 原样落库；
 *   UI 端按字符串完全匹配分组，不在枚举内 → 'other' 分组（**不**自动 normalize）
...
/** §D7：status 严格 3 态。agent 永不修改；UI 端 IPC zod 严格 enum reject 其他值。 */
...
/**
 * §D6：kind 软枚举（5 个推荐值）+ free-form fallback。
...
/**
 * §D16：append 子表行。每次 append_issue_context 调用 INSERT 一行；UI detail 视图按
 * appendedAt asc 渲染（read-only — agent 写的现场用户不改）。
 */
export interface IssueAppendix {
  ...
  /** 写入时 caller sid 快照（始终 == issue.sourceSessionId 因 §不变量 3）；session GC 后 SET NULL */
