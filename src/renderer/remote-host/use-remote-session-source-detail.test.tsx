...
describe('useRemoteSessionSource detail and mutation fencing', () => {
  ...
  it('submits the revision captured by the pending presentation', async () => {
    ...
    vi.mocked(window.api.listRemoteHostPendingIndex).mockResolvedValue({
      buckets: [{
        session: legacyRemoteSessionPresentation(session('same-session', 'remote-a list')),
  ...
  it('uses explicit cursors to append bounded remote pages without replacing prior rows', async () => {
    vi.mocked(window.api.listRemoteHostSessions).mockImplementation(async (request) => ({
      sessions: request.includeArchived
        ...
        : [{
            ...
            status: request.cursor ? 'dormant-finished' : 'active-idle',
            createdAt: 1,
    ...
    expect(hook.result.current.presentationCounts).toBeNull();
