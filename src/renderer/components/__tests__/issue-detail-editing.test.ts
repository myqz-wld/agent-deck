...
// deep-review H1 LOW（regression）：renderer 端前置校验，与 IPC zod min(1) + repo trim 守门对齐。
// 修前（无此函数）清空必填字段点保存 → IPC reject → 旧共用 error 路径摧毁整表单。
describe('validateEditing — 提交前 renderer 端校验', () => {
  const base = toEditing(makeIssue());
  it('合法 editing → null（通过）', () => {
    expect(validateEditing(base)).toBeNull();
  });
  it('空标题 → 报错（trim 后为空也拦）', () => {
    expect(validateEditing({ ...base, title: '' })).toBe('标题不能为空');
    expect(validateEditing({ ...base, title: '   ' })).toBe('标题不能为空');
  });
  it('空描述 → 报错', () => {
    expect(validateEditing({ ...base, description: '  ' })).toBe('描述不能为空');
  });
  it('空类型 → 报错', () => {
    expect(validateEditing({ ...base, kind: '' })).toBe('类型不能为空');
  });
  it('labels > 16 项 → 报错（与 IPC .max(16) 对齐）', () => {
    const seventeen = Array.from({ length: 17 }, (_, i) => `l${i}`).join(', ');
    expect(validateEditing({ ...base, labels: seventeen })).toBe('标签最多 16 个');
  });
  it('单 label > 64 字符 → 报错（与 IPC 单项 .max(64) 对齐）', () => {
    expect(validateEditing({ ...base, labels: 'a'.repeat(65) })).toBe('单个标签最长 64 字符');
  });
  it('恰好 16 项 / 单项 64 字符 → 通过（边界）', () => {
    const sixteen = Array.from({ length: 16 }, (_, i) => `l${i}`).join(', ');
    expect(validateEditing({ ...base, labels: sixteen })).toBeNull();
    expect(validateEditing({ ...base, labels: 'a'.repeat(64) })).toBeNull();
  });
});
