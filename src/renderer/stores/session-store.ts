...
interface State {
  ...
  setPendingRequestsAll: (
    map: Record<string, { permissions: PermissionRequest[]; askQuestions: AskUserQuestionRequest[] }>,
  ) => void;
...
export { EMPTY_REQUESTS, EMPTY_ASK_QUESTIONS };
...
export const useSessionStore = create<State>((set) => ({
  ...
  setPendingRequests: (sessionId, permissions, askQuestions) =>
    set((state) => {
      const p = new Map(state.pendingPermissionsBySession);
      ...
      return { pendingPermissionsBySession: p, pendingAskQuestionsBySession: a };
    }),
  ...
  setPendingRequestsAll: (map) =>
    set(() => {
      ...
      for (const [sid, { permissions, askQuestions }] of Object.entries(map)) {
