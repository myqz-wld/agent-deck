...
> 本文件是 agent-deck 仓库级共享 SSOT：仓库基础、基础目录架构、改动后必做、plan/review 生命周期、review 过期规则、文件大小护栏、项目特定约定（design invariant）、验证流程和打包与本地安装。Codex 对偶入口是 `AGENTS.md`；共享仓库规则以本文件为准，`AGENTS.md` 只补 Codex 入口差异，避免双写漂移。
>
