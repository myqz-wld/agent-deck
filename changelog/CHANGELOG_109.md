...
Plan `model-wiring-and-handoff-20260514`：闭环 model 链路三处不闭环点 — agent frontmatter `model` 字段在 teammate spawn 路径下死字段 + summarize/hand-off model 选择只能通过 env 配（UI 无暴露） + codex session hand-off 借用 claude SDK + sonnet。详细 plan 见归档 [`plans/model-wiring-and-handoff-20260514.md`](../plans/model-wiring-and-handoff-20260514.md)。
