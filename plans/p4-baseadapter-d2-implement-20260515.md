...
2. lead 调 `mcp__agent-deck__archive_plan({ plan_id: "p4-baseadapter-d2-implement-20260515", worktree_path: "/workspace/example/agent-deck/.claude/worktrees/p4-baseadapter-d2-implement-20260515" })` 自动收尾(ff merge worktree branch → main + frontmatter 改 status=completed + 同步 plans/INDEX.md + commit + worktree remove + branch -D + caller session shutdown)
