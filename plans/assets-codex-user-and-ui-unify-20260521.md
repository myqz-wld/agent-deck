...
worktree_path: "/workspace/example/agent-deck/.claude/worktrees/assets-codex-user-and-ui-unify-20260521"
status: "completed"
...
1. `Bash: cat /workspace/example/agent-deck/.claude/plans/assets-codex-user-and-ui-unify-20260521.md`
...
3. **先检查 worktree 是否已存在**(Step 1.5 reviewer-codex R2 MED-B 修订):
   ```bash
   git -C /workspace/example/agent-deck worktree list --porcelain | grep -A 1 'worktree.*assets-codex-user-and-ui-unify-20260521' || echo 'WORKTREE_MISSING'
   ```
4. 进 worktree(adapter 分流;按 step 3 输出选 fresh-create vs reuse-existing 分支):
   - **claude-code adapter**(任意状态):
     - **WORKTREE_MISSING**: 走 user CLAUDE.md §Step 2 EnterWorktree CLI stale base bug 主路径 (b) — 先 Bash 显式建 worktree(必须含 base_commit `<commit-ish>`,不能省略否则用 origin/main fallback 撞 stale base bug):
       ```bash
       git -C /workspace/example/agent-deck worktree add -b worktree-assets-codex-user-and-ui-unify-20260521 /workspace/example/agent-deck/.claude/worktrees/assets-codex-user-and-ui-unify-20260521 1fed5d8caca4b39a48841901fa237ab12b8ec862
       ```
       然后 `EnterWorktree(path: "/workspace/example/agent-deck/.claude/worktrees/assets-codex-user-and-ui-unify-20260521")`
     - **worktree EXISTS**(接力场景): 直接 `EnterWorktree(path: "/workspace/example/agent-deck/.claude/worktrees/assets-codex-user-and-ui-unify-20260521")`(不能再调 `git worktree add` 撞 path/branch already exists)
