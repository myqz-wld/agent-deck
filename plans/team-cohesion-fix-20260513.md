...
worktree_path: /workspace/example/agent-deck/.claude/worktrees/team-cohesion-fix-20260513
...
- 已进 worktree `/workspace/example/agent-deck/.claude/worktrees/team-cohesion-fix-20260513`
...
1. `Bash: cat /workspace/example/agent-deck/.claude/plans/team-cohesion-fix-20260513.md` 全文（**严禁 Read 工具**，详 ~/.claude/CLAUDE.md 「Step 3 接力姿势 cold start callout」）
2. `EnterWorktree(path: "/workspace/example/agent-deck/.claude/worktrees/team-cohesion-fix-20260513")` 进同一 worktree
3. `git log --oneline -5` 确认 HEAD 是 Phase B7 commit 或之后；`ls /workspace/example/agent-deck/.claude/worktrees/team-cohesion-fix-20260513/node_modules` 确认 symlink 在（如不在执行 `ln -sf /workspace/example/agent-deck/node_modules /workspace/example/agent-deck/.claude/worktrees/team-cohesion-fix-20260513/node_modules`）
...
**注意路径陷阱**：进 worktree 后所有指向**代码资产**的路径必须含 `.claude/worktrees/team-cohesion-fix-20260513/` 前缀（按 ~/.claude/CLAUDE.md「Step 1 worktree」节末段路径陷阱条款）。plan 文件本身路径不变（`/workspace/example/agent-deck/.claude/plans/team-cohesion-fix-20260513.md`，主仓库 .claude/plans/ 下，**不**在 worktree 内）。
...
- 相关 plan：`/workspace/example/agent-deck/plans/deep-review-flow-fix-20260512.md`（已 completed，本 plan 是其 Phase B/C 遗漏的下半场）
