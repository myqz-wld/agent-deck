...
worktree_path: /workspace/example/agent-deck/.claude/worktrees/mcp-handoff-fix-and-skill-timer-20260514
status: completed
base_commit: 5db9844
base_branch: main
...
- [x] **B3. user CLAUDE.md 加「§Step 2.5 何时主动 hand off」（H6+D4）** — done by sid `<this-sid>` on 2026-05-14, commit `0cc4f79` (worktree 改) + user home 直改 `/workspace/example/.claude/CLAUDE.md`. 新节客观信号 + 用户语义信号 + 例外
...
按 plan 进入 worktree（`EnterWorktree(path: "/workspace/example/agent-deck/.claude/worktrees/mcp-handoff-fix-and-skill-timer-20260514")`）→ 找 plan checklist 下一个未勾 step（**B1**）→ 直接动手。
...
3. **B3** ⚠️ **改的是 user home（不在 worktree）**：`/workspace/example/.claude/CLAUDE.md` 在 §复杂 plan 内插入新子节「§Step 2.5 何时主动 hand off（lead 自检触发）」。触发条件：context ≥ 60% / 完成独立 phase / worktree 干净 / plan 已写好下一步 + 用户语义信号「告一段落 / 先停一下 / 累了 / 先这样」；命中即默认走 §选项 B（K2 baton），plan 未填好 / worktree dirty 是例外
...
11. **B11**：`/workspace/example/.claude/templates/reviewer-{claude,codex}.sh.tmpl` 同步成 `$TMPDIR/...` mktemp 形式 (M8) — 这与 B5 user home 改动配套
...
- user CLAUDE.md: `/workspace/example/.claude/CLAUDE.md`（B3 / B5 改）
- 外部 reviewer 模板: `/workspace/example/.claude/templates/reviewer-{claude,codex}.sh.tmpl`（B11 改）
