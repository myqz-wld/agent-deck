...
**claude CLI version**: 安装在 `/workspace/example/.nvm/versions/node/v24.10.0/bin/claude`
...
- `additionalDirectories: ['/workspace/example/.claude', '/workspace/example/.codex', '/tmp']`(claude 读 OAuth creds + 写 session jsonl + Test 1/2 工作区)
...
spike3 的 `additionalDirectories` 只含 `/workspace/example/.claude`(够用 spike3 的 "say hi" 因为没 fs 操作)。spike4 因为 Test 1/2 涉及 `/tmp/hello.txt` 读,必须显式加 `/tmp`,否则:
- 外层 codex sandbox 不准 claude 子进程 spawn 的 `cat` 访问 `/tmp`(`workspace-write` 默认只准 `workingDirectory` 即 worktree)→ `cat` 撞 sandbox-exec 拒读
- 实测加了 `/tmp` 后 PASS

**对 P4 Step 4.2/4.3 实施影响**:`additionalDirectories` 至少需要 `['/workspace/example/.claude', '/workspace/example/.codex', '/tmp']`(或更精确按 reviewer 实际读写路径调)。reviewer-claude wrapper 让 claude 读项目源码做 review 时,`additionalDirectories` 必须包含项目根(`workingDirectory` 自动允许 + workspace-write 默认写权)。
...
- **P3 options-builder default 的 additionalDirectories**:已是 `['/workspace/example/.claude', '/workspace/example/.codex']`(P3 Step 3.5 落地的 spike3 推荐值)。**spike4 发现新需求 — 还需加 `/tmp`**(reviewer-claude wrapper 可能写中间文件 `/tmp/<input>.txt` `/tmp/<output>.txt`)。**v4.1 follow-up**:P3 Step 3.5 的 default `additionalDirectories` 可在 P4 Step 4.3 wrapper body 落地时一并补 `/tmp`,或在 P5 Step 5.3 fix review finding 时补
