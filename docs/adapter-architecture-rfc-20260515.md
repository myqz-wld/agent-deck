...
sign_off:
  chapter_1: accepted (Option D2)
  chapter_2: accepted (Option D 重写 + E 重写)
  chapter_3: accepted (范围定义 + 双类约定)
---
...
|---|---|---|---|
| 1 | P4 BaseAdapter / CreateSessionOptions 拆判别联合 + typed registry binding (Option D2) | ✅ accepted | RFC §"实施 follow-up 决策记录" Chapter 1 stub |
| 2 | 跨 adapter sandbox 继承（Option D 重写 string enum + Option E 重写 warnings 字段） | ✅ accepted | RFC §"实施 follow-up 决策记录" Chapter 2 stub |
| 3 | scheduler 命名 convention + 范围定义 + 双类周期 settings 约定 | ✅ accepted | 不需 plan（下次加新 scheduler 引用本 RFC） |
...
| 3 | 3.1 RFC 修订按 R1 反馈 | ✅ 完成（commit 7e4bec6） |
| 3 | 3.2 用户 sign-off 每章决策 | ✅ 完成（三章全 accepted） |
| 4 | 4.1+4.2 follow-up plan 决策记录 | ✅ 完成（RFC 内一节，详下文） |
| 4 | 4.3 Chapter 3 不需 plan | — |
| 5 | 5.1 RFC 归档（已在 docs/） | ✅ 完成 |
| 5 | 5.2 REVIEW_X.md（可选） | ⬜ 待用户决定 |
| 5 | 5.3 CHANGELOG_X.md + plans/INDEX.md | ⬜ 待 |
| 5 | 5.4 archive_plan | ⬜ 待 |

---

## 实施 follow-up 决策记录（sign-off 后落地）

每章 sign-off 决策落地为「未来 implementation plan 的触发条件 + 决策摘要」。**不单独建 stub plan 文件**（避免 plans/ 目录污染）— 未来真触发时新建 implementation plan 在 frontmatter 引用 `parent_rfc_id: adapter-architecture-rfc-20260515` + `parent_rfc_chapter: <N>`，按本节决策摘要直接动手。

### Chapter 1 — Option D2 实施 plan stub

- **触发条件**：parity-plan Phase A+B 完成收口（worktree-cross-adapter-parity-20260515 archive_plan）+ 与 parity-plan owner 协调 `extraAllowWrite` 字段最终归属
- **决策摘要**：
  - 拆 `CreateSessionOptions` 为 4 union arm + 4 interface（ClaudeCreateOpts / CodexCreateOpts / PtyCreateOpts × 2 共享）
  - typed registry overload + `buildCreateSessionOptions` builder helper + exhaustive switch
  - 5 处生产 caller 改 typed registry：spawn.ts:230 / cli.ts:268+315 / ipc/adapters.ts:174 / ipc/sessions.ts:133
  - hand-off 真实文件 hand-off-session.ts:281-303（非 hand-off-session-impl.ts）
  - 可选 Option C `delegateOrThrow` helper 顺手做
- **预估工作量**：~+165 / -140 行（1 个 PR 收口，详 RFC §1.5）
- **未来 implementation plan id 建议**：`p4-baseadapter-d2-implement-<YYYYMMDD>`

### Chapter 2 — Option D 重写 + Option E 重写实施 plan stub

- **触发条件**：用户实际报「跨 adapter spawn sandbox 配错」bug，或主动要求实施跨 adapter 安全意图保留
- **决策摘要**：
  ...
  - spawn handler 实装 string enum 映射（仅 strict→read-only / workspace-write→workspace-write；off / danger-full-access 不映射；`allow_unrestricted_mapping: true` 才走完整平凡映射）
  - emit message 到 lead session 双发 warnings（不止 console.warn）
  - hand-off-session.ts:281-303 同步透传逻辑（与 spawn handler 字面镜像）
  - 单测：spawn 4 场景 × 3 inherit_sandbox 值 + hand-off passthrough
- **预估工作量**：~+175 行（详 RFC §2.5）
- **未来 implementation plan id 建议**：`cross-adapter-sandbox-inherit-<YYYYMMDD>`

### Chapter 3 — 无独立 plan

下次加新 scheduler 时直接按 RFC §3.3 命名 convention + §3.3.4 双类周期 settings 约定，commit message 引用本 RFC Chapter 3 即可。既有 `LifecycleScheduler` / `TeamLifecycleScheduler` 不重命名（reviewer 已 ack 不做）。Summarizer / UniversalMessageWatcher 不属本 RFC 范围（参 §3.1.2 范围定义），命名按其语义不强制 Scheduler 后缀。
