...
[Team T = { lead: claude-session-A, teammate: codex-session-B, teammate: aider-session-C }]
                                          ↑
[claude-session-A] —MCP→ mcp__agent_deck__send_message(target_session: codex-B, team_id: T, text: ...)
                                          ↓
...
- **不**接管「用户在 Claude Code CLI 内自然语言起的 team」的可视化。CLI 内自起的 team 走
  ...
  `mcp__agent_deck__*` 5 个 tool 在 Claude 会话内编排 teammate。
- **不**在本 ADR 内重新定义 agent-deck-mcp 5 tool 的 wire schema **整体**（已在 R2 ADR §3 锁定）。
...
| `TeamDelete()` | `mcp__agent_deck__shutdown_session` 各 member<br>+ IPC `agent-deck-team:archive`（不删历史，标 archived） | 不再 cascade 删 fs；纯 DB 操作 |
...
前不重新校验 caller —— message 已在 enqueue 时（caller 调 `mcp__agent_deck__send_message` 或
...
teammate 通过 user prompt 操纵 LLM 调 `mcp__agent_deck__send_message` 给同 team 其他 member
...
1. agent-deck-mcp 5 tool（spawn / send / wait_reply / list / shutdown）
...
4. **老 deep-code-review SKILL workflow 废弃**。重写后用户需重新熟悉新 5 tool 路径。

### 10.2 替换路径

| 老操作 | 新操作 |
|---|---|
| 在 Claude 会话内说「创建 team T」让 Claude 自起 | 在 Claude 会话内调 `mcp__agent_deck__spawn_session({team_name:'T', ...})` |
| 在 Claude 会话内 `SendMessage(to:'reviewer', ...)` | `mcp__agent_deck__send_message({session_id: <reviewer-sid>, team_id?: ..., text:...})` |
| 等 teammate idle | `mcp__agent_deck__wait_reply({session_id:..., until:'idle', since_ts: ...})` |
| `TeamDelete()` | `mcp__agent_deck__shutdown_session` 各 member + IPC archive team |
...
- agent-deck-mcp 5 tool 自身的 wire schema 整体保留（仅 `send_message` 加 optional team_id；
...
- 用户首次升级版本后 Settings 顶端有持久 banner（不可隐藏）：「Claude 自起 team 已停止支持，
  请改用 mcp__agent_deck__spawn_session」附「查看迁移指南」按钮
