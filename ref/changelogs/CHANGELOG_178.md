# CHANGELOG_178 — plan issue-tracker-mcp-20260529 完整归档（agent 执行问题追踪机制 mcp tool + UI Issues tab + GC scheduler 全套）

## 概要

[plan `issue-tracker-mcp-20260529`](../plans/issue-tracker-mcp-20260529.md) 完整收口归档。新机制让 agent 执行过程中遇到「需要后续处理的问题」可即刻通过 `mcp__agent-deck__report_issue` / `append_issue_context` 上报到 SQLite issues 表 + UI 顶层「问题」tab 单独可见，用户可在 UI 端手动 triage（filter / 改字段 / 软删 / 「起新会话解决」一键 spawn 接力）。
