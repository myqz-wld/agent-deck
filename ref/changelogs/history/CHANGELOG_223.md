...
- 排查日志：`jsonl-fallback.ts` 在 precheck 判定 jsonl 缺失、即将走 fresh-cli fallback 时落一行
  `warn`（scope `claude-jsonl-fallback`），含 `emitContext / cwdFellBack / sessionId / cliSessionId /
  lookupId / cwd / 算出的 jsonl 绝对路径`。restart 路径 `cwdFellBack=false` → 该行就是「jsonl 本该
  在却没查到」的真信号，可直接 `ls jsonlPath` 区分「文件真不在（fork 子 jsonl 落盘 vs 连续快速重启
  时序竞态）」还是「precheck 路径算错」。
