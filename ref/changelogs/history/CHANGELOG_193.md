...
- **INFO-1**（pre-existing，非本 diff 引入）：`CODEX_AGENTS.md:275` hand_off_session 签名 `adapter?: "codex-cli"` 与 `schemas.ts:415` `.default('claude-code')` 不符，易被误读为默认值。先 `report_issue` 落 issue `a46ef1a8`（low），**本会话随即修掉**（见下 §Follow-up），issue 已 resolved。
...
## Follow-up（INFO-1 修复，issue a46ef1a8 resolved）

两端 hand_off_session 调用签名的 `adapter` 字段从单值改为展开 union：

- `adapter?: "codex-cli"`（codex）/ `adapter?: "claude-code"`（claude）→ 统一 `adapter?: 'claude-code' | 'codex-cli'`（与同签名里 `teamTaskPolicy` 全枚举写法对齐，消除「单值=默认」误读）
- 两端 §要点 列表新增 **adapter** 条目显式标默认值 `claude-code`：codex 端强调「省略一律起 claude-code session，与 caller adapter 无关，要接力到 codex 必须显式传 `'codex-cli'`」（codex lead 最易踩），claude 端同款精简版
- `schemas.ts:415` `.default('claude-code')` 未动——本次是文档对齐实现，非改实现
