...
> Plan `worktree-stale-base-bug-20260515`: Claude Code CLI v2.1.112 EnterWorktree builtin **stale base bug** R1 异构对抗 × **document-only** 收口（详 [REVIEW_38.md](../../reviews/history/REVIEW_38.md)）
...
- 新建 [`reviews/REVIEW_38.md`](../../reviews/history/REVIEW_38.md)（详记 root cause 全 trace、A/B 实测铁证、helper 实现独立验证、CLI tool description vs implementation 矛盾、R1 三态裁决（双方独立 ✅ 6 HIGH + 3 ✅ MED/LOW + 1 ❌ + 1 ❓ INFO）、workaround 4+1 候选评估 + ranking、触发条件矩阵（必中 / 不撞 / 边角）、应用层 fix 空间分析、upstream GitHub issue recipe、长远 fix 提案）
...
- Reproduction recipe + suggested fix（默认改 HEAD / 暴露 `fromHead` 公开 schema）+ issue 模板见 [REVIEW_38.md §Upstream report](../../reviews/history/REVIEW_38.md#upstream-report--anthropicsclaude-code-github-issue-recipe)
...
- 完整 root cause trace + R1 三态裁决 + workaround 评估 + upstream recipe + 长远 fix 提案 → [REVIEW_38.md](../../reviews/history/REVIEW_38.md)
- Plan 全文 + 调研日记 → [`plans/worktree-stale-base-bug-20260515.md`](../../plans/history/worktree-stale-base-bug-20260515.md)（archive_plan 后归档至 `<main-repo>/plans/`）
