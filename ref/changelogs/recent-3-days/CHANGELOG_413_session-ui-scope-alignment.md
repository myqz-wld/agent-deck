...
- Apply that opaque background and macOS window-control inset to every shared full-screen content
  surface, including messages, activity details, permissions, plans, diffs, issues, teams, and
  diagnostics.
- Add a shared input trigger variant so text-entry surfaces use the compact 24-pixel upper-right
  expand control while read-only detail cards retain their 44-pixel control.
- Remove drag resizing from every text area that already has a full-screen expand action, including
  hand-off, resolve-session, Issue, approval-feedback, and application-convention fields.
...
- Add regression coverage for shared overlay opacity and header inset, compact input triggers,
  retained detail-card trigger sizing, and removal of duplicate text-area resizing.
...
- `pnpm vitest run` for the shared expandable surface, new/resolve session, hand-off, Issue,
  application-convention, approval-feedback, pending-form, and composer suites (10 files and
  81 tests passed)
- `pnpm test` (468 files passed, one intentional smoke skipped; 4,007 tests passed, one skipped)
- `pnpm typecheck`
