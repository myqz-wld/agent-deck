...
worktree_path: "/workspace/example/agent-deck/.claude/worktrees/reviewer-codex-cross-adapter-20260519"
status: "completed"
...
1. `Bash: cat /workspace/example/agent-deck/.claude/plans/reviewer-codex-cross-adapter-20260519.md`（cold start 全文读 plan v6 含 Phase 0-4 完整收口实证 + Phase 5/6 待办)
2. `Bash: cat /workspace/example/agent-deck/.claude/plans/reviewer-codex-cross-adapter-20260519/spike-reports/spike1+2-cross-adapter-teammate-dispatch.md`（重点读 §Phase 0 Step 0.1a/0.2 root cause + 整套 fix v1/v2/v3 + phantom dep blocker 节,Phase 5 端到端回归会用到背景)
3. `EnterWorktree(path: "/workspace/example/agent-deck/.claude/worktrees/reviewer-codex-cross-adapter-20260519")` (worktree HEAD `48141ec`,用 `path` 不是 `name`)
...
8. **主 repo working tree 清理** (user 决定时机, 与 plan v3 §下一会话第一步 同款):
   ...
   - 选项 B:user 立即 `git -C /workspace/example/agent-deck checkout package.json pnpm-lock.yaml` 撤回 (保留 node_modules `@modelcontextprotocol/sdk` symlink)。**注意**:不要跑 `pnpm install` 撤改动 — pnpm 会发现 main package.json 与 .pnpm 不一致, 可能清理 top-level symlink 让 dev mode 重新撞 phantom dep
