...
worktree_path: /workspace/example/agent-deck/.claude/worktrees/ref-layout-full-migration-20260526
status: completed
...
`/workspace/example/agent-deck/CLAUDE.md` 实测 `grep -nE "changelog/|reviews/|conventions/| plans/| ref/plans"` 命中 **17 行**:
...
- [ ] **Pre-A.0 检测 main working tree 是否含 SSOT dirty**:
  ```bash
  git -C /workspace/example/agent-deck status --short \
...
- [ ] **Pre-A.1 stash dirty(含 untracked 新文件)**:
  ...
  cd /workspace/example/agent-deck
...
- [ ] **Pre-A.2 进 worktree 后 pop stash**:
  ...
  git -C /workspace/example/agent-deck/.claude/worktrees/ref-layout-full-migration-20260526 \
    stash pop
  # 应输出 "Changes not staged for commit" 含两文件
  ```
- [ ] **Pre-A.3 worktree 内 commit SSOT 改动**(让后续 plan A-F 改动建在新 SSOT 基线上):
  ```bash
  WT=/workspace/example/agent-deck/.claude/worktrees/ref-layout-full-migration-20260526
  git -C "$WT" add \
    resources/claude-config/CLAUDE.md \
...
- [ ] D.5 改项目根 `/workspace/example/agent-deck/CLAUDE.md` — **实测 17 处**(L24 / L30 / L31 / L33 / L35 / L38 / L39 / L41 / L43 / L45 / L50 / L64 / L146 / L152 / L153 / L157 / L160);用 BSD sed 批量(**R2 MED-3 完整命令模板**):
  ```bash
  sed -i '' \
    -e 's|`changelog/|`ref/changelogs/|g' \
    -e 's|`reviews/|`ref/reviews/|g' \
    -e 's|`conventions/|`ref/conventions/|g' \
    /workspace/example/agent-deck/CLAUDE.md
  ```
...
- [ ] **E.0(条件性 — 仅走 §Phase G tool 路径才必须)重启 dev 让 worktree 内 src/build 生效**(R2 HIGH-2 + R3 HIGH-4/HIGH-5 修订 — §不变量 10 enforce):
  ...
  - `cd /workspace/example/agent-deck/.claude/worktrees/ref-layout-full-migration-20260526 && pnpm dev`(在 worktree cwd 重启)
...
- [ ] **G-manual.4 ff-merge worktree branch 到 base_branch**(plan frontmatter `base_branch: main`):
  ```bash
  WT=/workspace/example/agent-deck/.claude/worktrees/ref-layout-full-migration-20260526
  MAIN=/workspace/example/agent-deck
...
- [ ] **H.3.5 verify plan frontmatter 已更新 status: completed + final_commit + completed_at**(R4-M_dual_1 fix 验证):
  ...
  grep -nE "status: completed|^final_commit:|^completed_at:" \
    /workspace/example/agent-deck/ref/plans/ref-layout-full-migration-20260526.md
...
- [ ] **H.4 verify git mv history 保留**(R3 MED-5 升级 — git rename detection 失败兜底):
  ...
  git -C /workspace/example/agent-deck log --follow \
    --format="%H %s" ref/changelogs/CHANGELOG_1.md | head -5
  # Fallback 1: 调高阈值
  git -C /workspace/example/agent-deck log --follow --find-renames=20 \
    --format="%H %s" ref/changelogs/CHANGELOG_1.md | head -5
  # Fallback 2: 显式 rename 查询(找 R 状态 commit)
  git -C /workspace/example/agent-deck log --all --diff-filter=R --raw \
...
- [ ] **H.5 worktree + branch 真删**(R3 MED-2 修法 — git show-ref 替代 grep -v 始终 success):
  ...
  ! git -C /workspace/example/agent-deck show-ref --verify --quiet \
...
1. `Bash: cat /workspace/example/agent-deck/.claude/plans/ref-layout-full-migration-20260526.md`(全文)
