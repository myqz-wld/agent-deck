...
base_branch: "main"
worktree_path: "/workspace/example/agent-deck/.claude/worktrees/resume-inject-raw-messages-20260601"
...
1. 若用户已 dev 实测确认两端 fallback 注入效果 OK → 走 archive_plan 收口：先 `ExitWorktree(action: "keep")` → `mcp__agent-deck__archive_plan({ planId: "resume-inject-raw-messages-20260601", worktreePath: "/workspace/example/agent-deck/.claude/worktrees/resume-inject-raw-messages-20260601", baseBranch: "main", changelogId: "194" })`
2. 若用户实测发现问题 → `EnterWorktree(path: /workspace/example/agent-deck/.claude/worktrees/resume-inject-raw-messages-20260601)` 进 worktree 修（worktree clean，4 commit 已落；改前先 `cat` 真实文件）
