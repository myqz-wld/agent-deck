...
worktree_path: /workspace/example/agent-deck/.claude/worktrees/deep-review-batch-a1-b-fixes-20260519
status: completed
...
- [ ] Step 0.1 — `git -C /workspace/example/agent-deck worktree add -b worktree-deep-review-batch-a1-b-fixes-20260519 /workspace/example/agent-deck/.claude/worktrees/deep-review-batch-a1-b-fixes-20260519`（避开 EnterWorktree CLI v2.1.112 stale base bug）
- [ ] Step 0.2 — `EnterWorktree(path: "/workspace/example/agent-deck/.claude/worktrees/deep-review-batch-a1-b-fixes-20260519")`（注意 path 不是 name）
...
- [ ] Step 5.8 — `mcp__agent-deck__archive_plan({plan_id: "deep-review-batch-a1-b-fixes-20260519", worktree_path: "/workspace/example/agent-deck/.claude/worktrees/deep-review-batch-a1-b-fixes-20260519", base_branch: "main", changelog_id: "X"})` 一行原子收口
...
1. `Bash: cat /workspace/example/agent-deck/plans/deep-review-batch-a1-b-fixes-20260519.md`（强制 cat 而非 Read，避免 cross-session jsonl cache 拿到旧版本）
2. **worktree 存在性兜底**（plan-review HIGH-5 codex 修订 + plan-review v2 NEW-MED 双方独立修订 — 旧版仅 path exists/missing 二档不覆盖 branch 已存在 / path exists 非 worktree / stale .git/worktrees metadata 等边角）：
   - 跑 `Bash: test -e /workspace/example/agent-deck/.claude/worktrees/deep-review-batch-a1-b-fixes-20260519 && echo EXISTS || echo MISSING`
   - **EXISTS**：先 sanity 检查是否真 worktree：跑 `Bash: git -C /workspace/example/agent-deck worktree list --porcelain | grep -A1 '/workspace/example/agent-deck/.claude/worktrees/deep-review-batch-a1-b-fixes-20260519'`
   ...
   - **MISSING**：进一步 branch 是否已存在判定（plan-review v2 NEW-MED reviewer-claude 修订）：
     - 跑 `Bash: git -C /workspace/example/agent-deck branch --list worktree-deep-review-batch-a1-b-fixes-20260519`
...
3. `EnterWorktree(path: "/workspace/example/agent-deck/.claude/worktrees/deep-review-batch-a1-b-fixes-20260519")`
