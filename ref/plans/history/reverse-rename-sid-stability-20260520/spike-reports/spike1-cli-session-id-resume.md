...
```bash
# /workspace/example/agent-deck/node_modules/.pnpm/@anthropic-ai+claude-agent-sdk@0.3.144_*/node_modules/@anthropic-ai/claude-agent-sdk/sdk.mjs
grep "resume" sdk.mjs
```
...
抽样 `~/.claude/projects/-workspace-example-agent-deck/` 下最近 mtime 5 个 .jsonl 文件:
...
encodeClaudeProjectDir('/workspace/example/agent-deck')
// → '-workspace-example-agent-deck'
```

实际目录 `~/.claude/projects/-workspace-example-agent-deck` 存在 ✅,与应用层 `src/main/platform.ts:23` 实现字面对齐。
