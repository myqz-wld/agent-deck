...
Status: Relay live acceptance complete; Full and real Feishu acceptance pending
...
- T0 through T4 are complete in the isolated `codex/feishu-one-click-server` branch. Feature implementation is commit `09c7676bd6095411177dd49664c678d77b6ae538`; follow-up commits `ef3610cc`, `08c558d3`, and `057c8f21` fix bounded slow-link upload, low-disk installation, and cross-process runtime reproducibility. Connection-control foundation commit `65f39ca1` and pre-release compatibility-cleanup commit `bdc7bdb3` remain separate cohesive history.
...
- Focused behavior/security coverage passed 21 files / 156 tests, followed by exact-expiry coverage at 2 files / 23 tests. The complete suite passed 962 files / 6,105 tests, with 2 files / 3 tests skipped by existing opt-in guards. `pnpm typecheck`, `pnpm build`, `pnpm verify:linux-headless`, `pnpm check:deployment`, Full/Relay/Feishu/Manager static checks, review-expiry, file-size, and `git diff --check` all passed.
...
- An authorized ARM64 Ubuntu Relay host passed official upgrade and independent verification at implementation release `git-057c8f21a130`. The clean-break metadata was recreated as schema v2, the existing Desktop forced command was canonicalized to surface `desktop`, the exact managed Relay unit restarted healthy, and no local process was terminated.
- The installed root-only CLI passed authority verification and disposable connection `issue`/idempotent reissue/`rotate`/idempotent rerotate/`revoke`/idempotent rerevoke. Both disposable credentials remain revoked, no test authorization remains installed, and the original Desktop credential remains active. `feishu check` and `feishu status` prove the runtime is installed and the unconfigured sidecar remains safely inactive.
