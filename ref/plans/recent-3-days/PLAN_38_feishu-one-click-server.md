...
Status: Relay schema-v2 cutover and hot-authority acceptance complete; rollback follow-up active; Full and real Feishu acceptance pending
...
Isolation: `<repo>/.agent-deck/worktrees/agent-deck-019ffbfd-37c-msrtkmb2` on branch `codex/feishu-one-click-server`
...
- Focused behavior/security coverage passed, including 8 files / 46 authority/deployment tests. The complete suite passed 963 files / 6,110 tests, with 2 files / 3 tests skipped by existing opt-in guards. `pnpm typecheck`, `pnpm build`, `pnpm verify:linux-headless`, `pnpm check:deployment`, Full/Relay/Feishu/Manager static checks, review-expiry, file-size, and `git diff --check` all passed.
...
- An authorized ARM64 Ubuntu Relay host completed the one-way clean break to schema-v2 release `git-4fd970044463`, generation 15. Its installed config/unit match the manager record, the journal is clear, and independent verification reports the exact digest-pinned container healthy. The retired combined config was restored to its generation-14 digest before an evidence-bound forward cutover; no compatibility reader was added and no local process was terminated.
- The installed root-only CLI passed authority verification and disposable connection `issue`/idempotent reissue/`rotate`/idempotent rerotate/`revoke`/idempotent rerevoke. A schema-v2 live test additionally proved `issued → active → revoked` projection through the directory bind without changing the Relay container start time. All disposable credentials are revoked, no test authorization remains installed, and the original Desktop credential remains active. `feishu check` and `feishu status` prove the runtime is installed and the unconfigured sidecar remains safely inactive.
...
- Final records: `CHANGELOG_612_feishu-server-one-click.md`, `REVIEW_247_feishu-server-one-click-acceptance.md`, `REVIEW_248_feishu-relay-live-acceptance.md`, and `PLAN_38_feishu-one-click-server.md`.
