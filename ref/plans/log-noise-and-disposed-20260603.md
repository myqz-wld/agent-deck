...
worktree_path: "/workspace/example/agent-deck/.claude/worktrees/log-noise-and-disposed-20260603"
status: "completed"
base_commit: "34ab566"
base_branch: "main"
...
1. `Bash: cat /workspace/example/agent-deck/.claude/plans/log-noise-and-disposed-20260603.md`（必 cat 详会话记录）
2. Step 1 建 worktree（先 bash 创 + EnterWorktree(path:)，不用 `EnterWorktree(name:)`）：
   ```bash
   git -C /workspace/example/agent-deck worktree add -b worktree-log-noise-and-disposed-20260603 /workspace/example/agent-deck/.claude/worktrees/log-noise-and-disposed-20260603
   EnterWorktree(path: "/workspace/example/agent-deck/.claude/worktrees/log-noise-and-disposed-20260603")
   ```
...
- **worktree 内跑 vitest 必须用主仓库 Electron binary**（详 message-retention plan §已知踩坑）：worktree 无 node_modules，跑测试走 `/workspace/example/agent-deck/node_modules/.pnpm/electron@33.4.11/.../Electron` + `ELECTRON_RUN_AS_NODE=1`，**勿 symlink node_modules 进 worktree** 会被 vitest .vite 缓存污染。
