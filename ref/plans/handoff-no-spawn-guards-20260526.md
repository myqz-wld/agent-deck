...
worktree_path: "/workspace/example/agent-deck/.claude/worktrees/handoff-no-spawn-guards-20260526"
status: "completed"
...
1. `Bash: cat /workspace/example/agent-deck/.claude/plans/handoff-no-spawn-guards-20260526.md` 全文
2. 若 §当前进度 仍 Step 1.5 R3 未通过 → invoke `/agent-deck:deep-review` (args: `{kind: 'plan', paths: ['/workspace/example/agent-deck/.claude/plans/handoff-no-spawn-guards-20260526.md']}`) 走对应轮次复审
3. 若 §当前进度 Step 1.5 R3 已通过(双 reviewer "可合" 共识)→ 用户 confirm 进 worktree 后,**互斥二选一**:

   **选项 A — claude 端 手工 git + builtin EnterWorktree**(两步):
   ```bash
   git -C /workspace/example/agent-deck worktree add -b worktree-handoff-no-spawn-guards-20260526 /workspace/example/agent-deck/.claude/worktrees/handoff-no-spawn-guards-20260526 50b46ffbf20b11020153594b041c25937bd9308e
   ```
   ...
   然后进 worktree:
   ```
   EnterWorktree(path: "/workspace/example/agent-deck/.claude/worktrees/handoff-no-spawn-guards-20260526")
   ```
   ...
   **选项 B — codex 端 mcp 创建 worktree + 手工 cwd 切换**(R1 HIGH-1 修法 — codex MCP 不自动切 cwd):
   ...
   ⚠️ **codex 端必须**在 Step 2.1-2.7 后续每个 shell / apply_patch 调用使用 worktree 绝对路径 `/workspace/example/agent-deck/.claude/worktrees/handoff-no-spawn-guards-20260526/...` 或 `git -C /workspace/example/agent-deck/.claude/worktrees/handoff-no-spawn-guards-20260526 <cmd>`。**绝不**使用相对路径(会落到 codex spawn 时的 cwd,即主仓库 `/workspace/example/agent-deck`,污染 main working tree)。
   ...
   **选项 A 失败 cleanup**(纯 git,无 DB marker 副作用):
   ```bash
   git -C /workspace/example/agent-deck worktree remove /workspace/example/agent-deck/.claude/worktrees/handoff-no-spawn-guards-20260526 --force
   git -C /workspace/example/agent-deck branch -D worktree-handoff-no-spawn-guards-20260526
   ```
   ...
   **选项 B 失败 cleanup**(MCP 路径,优先用 `exit_worktree` 一并清 marker + worktree + branch — 因 `enter_worktree` 成功步骤 8 写 `cwd_release_marker`,纯 git cleanup 不清 DB marker):
   ```
   mcp__agent-deck__exit_worktree({ action: "remove", worktree_path: "/workspace/example/agent-deck/.claude/worktrees/handoff-no-spawn-guards-20260526", discard_changes: true })
   ```
