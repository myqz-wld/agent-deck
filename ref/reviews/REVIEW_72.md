...
- **验证**（lead 本 worktree 内实跑铁证）：`git rev-parse --show-toplevel` → worktree 自身；`git rev-parse --git-common-dir` dirname → 真 main repo（`/workspace/example/agent-deck`）。grep 确认 enter 是 4 个 impl 中唯一用 show-toplevel 的。
