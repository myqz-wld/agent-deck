...
### ✅ MED-3 codex 打包版 pathDirs 丢失（reviewer-codex 提出 → 核为 pre-existing → 已修 live bridge 路径）
...
处置：先 `report_issue`（id 8c116860）记录 → 后续同会话**修复 live bridge 路径**（commit 6da8db5）：codex-binary.ts 加 `resolveBundledCodexPathDirs()` + `prependBundledCodexPathDirs(env)`（与二进制双布局对齐 new=codex-path/ legacy=path/ + existingDirs 语义 + 复刻 SDK prependPathDirs 去重），ensureCodex 走 bundled 二进制时注入 envOverride.PATH；用户自填 codexCliPath 不注入；oneshot pool 刻意不补（不触发 rg + 继承 process.env，加注释）。+5 回归 case。**残留**：未端到端实测打包版退化行为（修法对齐 SDK 源码逻辑），下次打包后 spot-check codex 文件搜索类 shell tool。append 到 issue 8c116860。
...
- `pnpm test` 1099 passed / 197 skipped（skip 为 SQLite binding ABI 门控，pre-existing）。
- 新增 codex-binary-layout 10 case（二进制双布局 5 + pathDirs 5）+ issues IPC 32 case 全绿。

## 收口判定

reviewer-claude 环境性缺席（非漏审，CLI 系统性 hang）→ 单 reviewer（codex）+ 主 agent 现场验证下，0 未决 HIGH/MED（MED-3 已修 live bridge + append issue / INFO 留产品决策）。本轮可合。LOW/INFO 已处理或转跟踪。
