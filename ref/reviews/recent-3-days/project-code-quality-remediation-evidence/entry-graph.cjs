...
const out = '.ref/reviews/2026-09-04-quality-remediation';
fs.mkdirSync(out, { recursive: true });
const tracked = [...new Set(cp.execFileSync('git', ['ls-files', '--cached', '--others', '--exclude-standard', '-z'], { encoding: 'utf8' }).split('\0').filter(p => p && fs.existsSync(p)))];
