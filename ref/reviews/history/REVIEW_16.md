...
最终静态 audit 发现：用户的 `~/.claude` 是 symlink 指向 `/workspace/example/.claude-default`，chokidar 在 macOS fsevents 下回 handle 函数的 `filepath` 是 realpath 化路径，与代码里 `getInboxPath()` 拼出的 raw symlink path 严格 `!==` 比较永远 true → handle 永远 early return → emit 永远不 fire。
...
**关键证据来源**：
- `python3 -c "import os; print(os.path.realpath('~/.claude/teams/.../team-lead.json'))"` → `/workspace/example/.claude-default/...` 与 raw `/workspace/example/.claude/...` 不等（铁证 symlink）
