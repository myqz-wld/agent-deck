...
- `CODEX_AGENTS.md`：替换资源占位符后同步进 `~/.codex/AGENTS.md` 的应用 marker 段。设置面板保存的用户副本落 `<userData>/agent-deck-codex-agents.md`，存在时覆盖内置文件；内置文件缺失时显式报错，不回落到 claude-config 侧。
...
- `agent-deck-plugin/skills/*/SKILL.md`：替换资源占位符后安装到 `~/.codex/skills/agent-deck/<skill>/SKILL.md`；源目录删掉的 skill 安装目录同步删除。
