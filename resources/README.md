...
- Agent Deck 打包资源必须自闭环：用户不安装任何 `.claude/skills` / `.codex/skills` 时，内置应用约定、reviewer agents、skills 与 MCP tool descriptions 仍要完整可用。用户环境里的 skills 只能增强维护效率，不能成为打包资源的必要依赖。
- project templates、changelog/review/convention 组织规则、flow/architecture diagram 维护、file-size guardrail 与 review expiry 脚本不打进 Agent Deck baseline；维护本仓库时按根 `CLAUDE.md` 最小规则和 `ref/` 现有格式执行。
- 改长生命周期 prompt 资产前，按根 `CLAUDE.md` 与 `resources/claude-config/CLAUDE.md` / `resources/codex-config/CODEX_AGENTS.md` 的“提示词资产维护”章节做自检；根 `README.md` / `AGENTS.md` / 本文件也算 prompt 资产。
