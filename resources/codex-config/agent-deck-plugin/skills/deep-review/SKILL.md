...
> **TL;DR**：把 reviewRoot 外的文件 cp 到 reviewRoot 内 cache 目录（让 reviewer SDK sandbox 能读；reviewRoot 见上方定义，可为 repo root / worktree root），review 完自动清。每次 SKILL 调用建独立子目录避免并发互踩。**详细 7 步**：
...
> 完整契约 SSOT：应用 CODEX_AGENTS.md §决策对抗 §三态裁决 + §Finding 输出契约（codex 端 baseline，已注入你的 system prompt）。
...
| 4 | 反驳轮（仅 HIGH 单方独有）：`send_message({sessionId: B-sid, teamId, text: '<A 的 finding 全文> 请独立反驳', replyToMessageId: <Round 1 messageId>})` 把 A 的 finding 给 B 反驳（reverse 同理）→ 同 Step 2，等 B 的反驳 reply 自动注入 lead conversation | 同一条 finding 只反驳一次；反驳后仍不能定 → lead 自己验证；还不行 → 降级非 HIGH |
...
> 完整契约 SSOT：应用 CODEX_AGENTS.md §Finding 输出契约（codex 端 baseline）。
...
本 SKILL = **多轮**深度 review × fix × 反驳轮编排（teammate 模式，跨轮 context 持久化、反驳轮被反驳方有自身上轮推理链）。**单次决策对抗**（单点判定 / plan 评审 / 约定升级）走双外部 CLI 直接起（详 应用 CODEX_AGENTS.md §决策对抗 主路径），零 SDK 状态。两个场景两个姿势，不混用。
