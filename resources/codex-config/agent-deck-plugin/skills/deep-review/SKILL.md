...
- **kind='plan'**: 复杂 plan 写完先评审 — 评审 plan design / 流程一致性 / 不变量定义 / step 行级 reference / 测试矩阵覆盖度
...
6. **SKILL 启动 step 0 双自检**：
   ...
   - **`.gitignore` 自检**：`shell: grep -q '^\.deep-review-cache/' <reviewRoot>/.gitignore` 失败且 `ack_cache_unignored` 不是 true → warn caller「`.gitignore` 缺 `.deep-review-cache/` entry，cache 文件可能被 commit；请加 entry 或接受风险继续」并 stop for explicit consent。批处理 / 自动调度场景必须传 `ack_cache_unignored: true` 才能跳过自检并接受 cache untracked 风险。
