...
| 2 | **告诉 user**「已派 2 个 reviewer 跑 review，UI 实时显示进度，reply 来了会作为下一轮 user message 注入；我收到后继续裁决」**然后结束当前 turn**。reviewer 跑完调 `send_message + replyToMessageId` → reply 自动注入 lead conversation。**Codex lead 不主动 poll** | 两份独立 finding 在下一轮注入 |
| 3 | 三态裁决（§三态裁决）：双方一致 → ✅；任一 CRITICAL/HIGH → Step 4 反驳轮；单方独有 MEDIUM → lead 自己 grep/cat 验证；LOW/INFO → 直接列 ❓ | — |
| 4 | 反驳轮（所有 CRITICAL/HIGH，**只一次**）：`send_message({sessionId: B-sid, teamId, text: '<A 的 finding 全文> 请独立反驳，禁止借机提其他 finding', replyToMessageId: <Round 1 messageId>})` → 结束当前 turn；B 反驳 reply 下一轮注入 | 同一条 finding 必须记录支持论与反驳论；反驳后仍不能定 → lead 自己验证；还不行 → 降为 MEDIUM 或更低 |
| 5 | **可选**一轮 fix：有 CRITICAL/HIGH 真问题 → lead fix；MEDIUM 由 lead 明确「本轮修 / 接受风险 / 记 follow-up」→ **复用同对** teammate 调 `send_message` 发 Round 2 prompt（带 `skip` = 上轮 ✅ fix 摘要 `已修：<filepath:line> <一句话改动> (commit <hash>)`）→ 结束当前 turn；reply 下一轮注入后回 Step 3。**收口**判定满足（0 CRITICAL/HIGH + 双方可合）即进 Step 6，**不强制多轮**；若仍 ≥ 1 CRITICAL/HIGH 或双方又抓 ≥ 5 条新真问题 → 升 `agent-deck:deep-review` 接管多轮挖深 | fix loop 期间**绝不 shutdown**（复用同对 teammate 保 mental model） |
| 6 | 收尾：`shutdown_session` × 2 + cleanup auto-cp cache（按 manifest 精确 rm，即使失败也尝试 rm 一遍）+ 输出 §Final Summary Report | shutdown 不删 events / messages，lead 仍可在裁决报告引用。收尾后必须给用户总结报告，不要只说 review 已完成。 |

> **lead 自然推进**：reply 走 adapter dispatch 自动注入 receiver SDK conversation flow → reviewer reply 到 lead 后成为下一轮 user-role message → Codex lead 当普通 user input 处理 → 继续裁决。

**收口判定**：双 reviewer 都「可合」+ 0 CRITICAL/HIGH。MEDIUM 必须有 lead 处置记录；LOW/INFO 留 follow-up。单方收口不算（漏审风险）。

## Final Summary Report

When simple review reaches 收口, 拒合, or escalation to deep-review, lead must deliver a final summary report before ending the workflow. Do not finish with only "done" or "review passed".

Report fields:
- Scope and review kind, including reviewed paths.
...
**单方独有分流**：CRITICAL/HIGH → §执行模板 Step 4 反驳轮；MEDIUM → lead 自己 grep / cat 验证（≤ 5min / ≤ 5 grep / ≤ 1 test，超就保留 ❓ 并降到 LOW/INFO）；LOW/INFO → 直接列 ❓。双方都说没问题 → ✅ 可合。
...
| reviewer 持续不 reply（≥ 30min 无 reply + nudge 后仍无）| 仅在 user 下一轮询问状态或达到 30min 阈值时调 `get_session(reviewerSid).lastEventAt` 检查：recent → 还在跑只是慢，告诉 user 再等等并结束 turn；非 recent → 卡死，提示真人去 PendingTab 或 shutdown 重 spawn |
...
本 SKILL = **单次** review + 可选一轮 fix（轻量异构对抗）；`agent-deck:deep-review` = **多轮**挖深 review × fix × 反驳轮（teammate 跨轮持久化）。两个 SKILL 都各自 inline §三态裁决 + §Finding 输出契约（各自 SSOT，不交叉引用应用 CODEX_AGENTS.md）。本 SKILL 跑完发现水比预期深（≥ 1 CRITICAL/HIGH 反复 / 跨多模块）→ 升 deep-review 接管。
