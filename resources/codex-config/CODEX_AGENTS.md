...
- 状态切换: `mcp__agent-deck__task_update({ task_id, status })`,枚举 `pending` / `active` / `completed` / `blocked` / `abandoned`(注意 `active` 替代 Claude Code 原生 `in_progress`)。**写权限**按 team_id 判定(team-bound → caller 必在 team / personal → caller==owner);跨 team 写 reject
- **列表查询三态分流** (v024 §D5):
  - 默认 `mcp__agent-deck__task_list()` → 返 `{ total, hasMore, tasks }`,visible scope = caller 可见 scope(caller 自己 personal ∪ caller 所在所有 active team 的 team task,一次 SQL OR 模式拿完;`hasMore: true` 表示 `tasks.length === limit` 可能还有,翻页传 `offset: prevOffset + tasks.length`)
