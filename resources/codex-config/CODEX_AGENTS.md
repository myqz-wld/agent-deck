...
**deny external caller**:sessionManager.close 是写操作 + caller=lead 反查需要真实 callerSessionId,绝不允许 stdio external client 调用(避免被恶意 mcp client 利用清理任意 team session)。
...
- **ShutdownAllTeammates 不调 reassignTaskOwner**:与 handOff caller→newSid 不对称。teammate 关闭后 task 仍在 teammate 名下,被 `LifecycleScheduler.historyRetentionDays` TTL GC 触发 `sessionRepo.delete` 时 CASCADE 删。teammate context 已死 task 本质无主,删干净是合理设计(配合 GC 主语义)。
