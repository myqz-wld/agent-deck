...
- `mcp__agent-deck__enter_worktree({ plan_id, base_commit?, base_branch? })`:创建 / 进入 worktree 目录 + 切当前 SDK session cwd
...
2. 从 frontmatter 拿 `worktree_path` + `plan_id` → 调 `mcp__agent-deck__enter_worktree({ plan_id, base_commit: <frontmatter.base_commit> })` 进 worktree(codex 无 native EnterWorktree CLI 必须走 MCP;`base_commit` 显式传 frontmatter 的 `base_commit` 避撞 EnterWorktree CLI stale base bug。schema 接受 `base_commit` / `base_branch` 两字段;旧文档曾用 `base` 字段已废弃)
