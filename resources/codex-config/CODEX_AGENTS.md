...
For a structured lease, success with `state: "waiting-tool-result"` accepts the reverse transition; it does not mean the worktree was already removed. Agent Deck observes the exact result, interrupts at the safe boundary, restores and confirms the original runtime/database cwd, checks references and dirty state again, then removes the worktree. `state: "completed-cleanup"` means a cleanup retry finished after cwd restoration; `state: "completed-legacy"` is the synchronous marker-only compatibility path. If cleanup remains pending, the session already runs from the original cwd: preserve the worktree, resolve the reported condition, and retry `exit_worktree`.
