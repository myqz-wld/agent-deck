...
应用环境跑 `deep-review` SKILL 时若 reviewer-claude wrapper teammate 失败(打包 claude binary 不可用 / OAuth 过期 / 嵌套 sandbox 拒读 / timeout),可走「合规兜底(仍异构)」:lead 自己 `shell` 起外部 claude CLI(按 reviewer-claude.md §claude CLI 调用模板填模板,lead 自己执行而非 wrapper teammate),与 reviewer-codex teammate 仍构成 Opus 4.7 vs gpt-5.5 异构对。
