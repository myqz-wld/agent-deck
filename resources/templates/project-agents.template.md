...
> Agent Deck 应用环境通用约定（输出 / 运行时 / review 对抗 / 工程地基）会随 SDK 会话自动注入，Claude / Codex 两端协议层语义对齐。

## 必读顺序

1. 先读 [`CLAUDE.md`](CLAUDE.md)，执行其中的仓库基础、项目特定触发、项目特定约定、验证流程（Claude / Codex 共用的项目 SSOT）。
2. 涉及应用内 Codex SDK 会话 / MCP / skill / prompt asset 时，遵守已注入的 Codex 应用环境约定。
3. 涉及 Claude / Codex 对偶资产时成对审计；adapter 工具差异允许措辞不同，协议语义不能单边漂移。
