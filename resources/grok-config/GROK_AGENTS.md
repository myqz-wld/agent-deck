...
Use Agent Deck's worktree tools when the task needs isolation. `enter_worktree` accepts only a named local base branch; `waiting-tool-result` is durable acceptance, and Agent Deck automatically switches the session after the provider observes that exact result, so do not issue `cd` or continue old-cwd work. Before normal exit, commit and push intended changes. `exit_worktree` adopts an existing legacy marker/path into the same restore-first flow and returns `waiting-tool-result`; `completed-legacy` means the target was already absent. Preserve a cleanup-pending worktree and retry after resolving the reported condition. Keep branches by default, and use `discardChanges` or `deleteBranch` only with the user's explicit authorization.
