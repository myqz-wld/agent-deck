...
> 本文件含 agent-deck 应用专属差异 + plugin 资产 inline 的关键工程实践(§复杂 plan workflow / §新项目工程地基 等已 inline,self-contained 不依赖 user CLAUDE.md 加载)。
>
> **加载范围**:本文件总是随应用打包注入到 SDK system prompt 末尾。`settingSources: ['user','project','local']` 的交互式 SDK 会话同时加载 user CLAUDE.md,本节关键工程实践与 user CLAUDE.md 互补不冲突;`settingSources: []` 的内部 oneshot(如间歇总结)不加载 user CLAUDE.md,本节 inline 内容仍可用。
...
**Why** (plan task-mcp-owner-session-id-rewrite-20260521 v023 + task-mcp-merge-into-agent-deck-mcp-20260521 合并 + **plan task-team-id-restore-20260525 v024 team_id stored 重设计**):
...
- **v024 重设计 — team_id 字段 stored**:tasks 表加回 `team_id TEXT NULL`(non-null = team-bound task / NULL = personal task)。team scope **从 derived 改回 stored** — 消灭 v023 「lead 多 team task 串流」根因(lead L 同时在 team A、team B,L 在 A 上下文 task_create 落 owner=L 无 team 标签 → team B 反查 ownerSessionIds 含 L → list 拿到 owner=L 全部 task 包括 A 专属那条)。v024 改成 team_id 标签判定,team 之间严格隔离
- **可见性 / 写权限 按 team_id 决定**(v024 §D3 — read/write 镜像):
  - `team_id != null` = team-bound task → caller 必须在该 team 是 active member(`agent_deck_team_members.left_at IS NULL AND agent_deck_teams.archived_at IS NULL` 双条件)才能 read/write
  - `team_id IS NULL` = personal task(first-class — 无 team caller 也能用 task)→ **仅 owner 可见可写**(不开放同 team 共享,避免 lead personal task 被同 team teammate 偷看 / 偷改)
- **task_get 严格 team-scoped 不再「跨 team 可读」**(v024 §D8):in-process lead 跨 team 看 teammate task / external mcp client 凭已知 task_id 查 task 两类 use case 都被 v024 推翻 — task_get 走与 task_update/delete 同款 deny external 对称语义。lead 想看跨 team teammate task 应走「让 lead 加入对方 team 作为 active member」或显式 IPC TaskListByTeam(若 lead 在该 team active)
- **personal task 是 first-class**:用户不在任何 team 时仍能用 task(典型场景:lead 起 reviewer pair 不必加 team 也想用 task 跟踪)。不传 `team_id` = personal default,与 caller 是否在 team 无关 — backend **不自动闭包**第一个 team(避免与「lead 多 team 串流」根因同型)
- `hand_off_session` baton 时按 `team_task_policy` 三态处理 task: 默认 `'clear-team'` 过继 ownership 同时清 team_id 变 personal / `'preserve-team'` 保留 team_id(caller 自负保证 adopt_teammates=true)/ `'skip'` 真删 team task(详 §hand_off 节 team_task_policy 三态使用模式)
- GC 自动: caller archive 后 sessions row 被 `LifecycleScheduler.historyRetentionDays` 物理删时, ON DELETE CASCADE 自动删 owner=caller 的 task — 无 backlog 累积问题(v024 加回 team_id 不复活该问题 — team 硬删时 tasks.team_id ON DELETE SET NULL 退化为 personal task,owner_session_id NOT NULL 仍兜底 GC)
- 原生 `TaskCreate` 只 in-process 当前 SDK session 可见,跨会话 / 跨 teammate 全部丢失,与 universal team backend 协作意图相违背

**How to apply**:
- **新建 personal task** (default,与 caller 是否在 team 无关): `mcp__agent-deck__task_create({ subject, description?, status?, priority?, blocks?, blocked_by?, labels? })` → owner_session_id 自动闭包当前 caller,team_id=NULL,**仅 caller 自己可见可写**
- **新建 team-bound task** (caller 必在该 active team): `mcp__agent-deck__task_create({ subject, team_id: <team-uuid>, ... })` — 不在该 team 是 active member 时 reject(防 caller 显式传错 team_id)。同 team active member 可见可写;**caller 显式传 team_id=B 时 ingest payload.teamName=B**(不漂移到 first active team — v024 §不变量 6)
- 状态切换: `mcp__agent-deck__task_update({ task_id, status })`,枚举 `pending` / `active` / `completed` / `blocked` / `abandoned`(注意 `active` 替代原生 `in_progress`)。**写权限**按 team_id 判定(team-bound → caller 必在 team / personal → caller==owner);跨 team 写 reject
- **列表查询三态分流** (v024 §D5): 
  - 默认 `mcp__agent-deck__task_list()` → caller 可见 scope(caller 自己 personal ∪ caller 所在所有 active team 的 team task,一次 SQL OR 模式拿完)
  - `task_list({ team_id_filter: <team-uuid> })` → 该 team 绑定的 task(caller 必在该 team active member,否则 reject)
  - `task_list({ team_id_filter: 'null-personal' })` → caller 自己的 personal task(字面量,与传 team-uuid 区分语义)
- 单个查询: `mcp__agent-deck__task_get({ task_id })` — **v024 严格 team-scoped read**(D3 镜像 write):team-bound task caller 必在 team active member / personal task caller==owner;不通过 → reject。**deny external caller**(external mcp client 拿到明确 error 而非 silent reject)
...
> ⚠️ 走自动 tool 起的新 session 同样按 §选项 A cold start 流程跑（cat plan + EnterWorktree + 自检 + 动手）。新 session 必须加载本节(§复杂 plan workflow inline 在本应用打包 CLAUDE.md 内)才知道 cold start prompt 含义 —— 应用 SDK 会话总是注入本文件,加载有保证。
...
- **hand-off 完全独立于 spawn-guards / 永不写 spawn-link**(plan handoff-no-spawn-guards-20260526 §D1/§D4/§D6 — **故意推翻** REVIEW_46/47「archive_caller=false 退化 normal spawn」修法):`hand_off_session` 内部调 spawn 时传 `handOffMode: true`(原名 `batonMode`,本 plan §D6 改名 + 语义升级),让 spawn handler **完全跳过** spawn-guards 三道防御(depth check + fan-out + spawn-rate)+ **永不写** `spawn-link`(`sessions.spawned_by`/`spawn_depth` 保持 null/0)。理由:hand-off 是「平级接力 + 接管 lead 身份」语义不是 spawn 派遣关系(`hand-off-session.ts:21-39` jsdoc 明文「不是派出小弟干活」),数据层不应记录 spawn-link 让 SessionList Phase C 树形分组错挂 teammate badge。**统一行为**:无论 `archive_caller` 值(default true / 显式 false)/ `adopt_teammates` 值(default false / true),hand-off 路径行为完全一致 — 新 session 在 SessionList 呈现为独立 root,不显示与 caller 任何 spawn 关系。**`archive_caller: false × N` 滥用风险**:caller 持 false × N 次起 N 个 session 是合法 power-user 路径(典型 lead 起多 hand-off 子任务自己仍想看 reviewer reply / debug 工具用例),应用层不阻止 — power-user 自负责任
...
- **新 session cold-start 5 步**（plan-driven 模式；本应用打包 CLAUDE.md 含 §复杂 plan workflow 让 cold start prompt「按 <plan-abs-path> 接力」可识别,self-contained 不依赖 user CLAUDE.md 加载）：① `Bash: cat <plan-abs-path>` 全文 → ② frontmatter 取 `worktree_path` → ③ `EnterWorktree(path: <worktree_path>)` 进 worktree（用 `path` 不用 `name` 避开 v2.1.112 stale base bug）→ ④ `git log --oneline -3` 自检 HEAD = frontmatter `base_commit` 或之后 → ⑤ 按 plan §下一会话第一步 直接动手。详细约定见本应用打包 CLAUDE.md §复杂 plan workflow §Step 3 接力姿势节
- **task 自动过继 + team_task_policy 三态**(plan task-mcp-owner-session-id-rewrite-20260521 v023 §D3 + deep-review Round 1 F1/F3 修法 + **plan task-team-id-restore-20260525 v024 §D4 三态升级**): spawn 完成 + 新 sid 落 DB + adopt 完成后、archive caller 之前, 按 `team_task_policy` 三态处理 caller owned task。**`reassignOwner` 不刷 `updated_at`** 让 list 默认排序保持稳定。
  - **`'clear-team'`** (default): `UPDATE tasks SET owner_session_id=newSid, team_id=NULL WHERE owner_session_id=callerSid` — 过继 ownership 同时清 team_id 变 personal。**Why default**:适用面最广,newSid 拿到的 task 都是 personal(team_id IS NULL),caller==owner 写权限路径直接生效;newSid 不必加任何 team 就能继续 own / read / write 全部过继的 task。typical baton 场景(adopt_teammates=false / 不传)首选,task 不丢
  - **`'preserve-team'`** (caller 自负责任): `UPDATE tasks SET owner_session_id=newSid WHERE owner_session_id=callerSid`(不动 team_id)— 过继 ownership 但保留 team_id。**Why this option**:caller 想让 newSid 接管 team-bound task 同时继承 team 上下文(典型与 `adopt_teammates=true` 配合使用让 newSid swapLead 接管 team 当 lead,team-bound task 仍指向同一 team newSid 可写)。**Caller 自负责任语义**:如果 newSid 没成为对应 team 的 active member(`adopt_teammates !== true` / args.team_name 没显式传该 team),newSid 撞 D3 写权限 reject — handler 不 hard reject 但 ok return `taskReassignment.policyWarning='preserve-team-unadopted-teams'` + `unadoptedTeamIds: string[]` 字段暴露差集 team_id 让 caller 看到「这些 team 的 task 仍归 newSid 但 newSid 不是 active member 无人可写」,决定是否 retry adopt 或接受降级
  - **`'skip'`** (清理场景): 单 transaction 4 步原子化:SELECT caller owned team task ids → 一次性 DELETE → cleanup blocks/blocked_by 引用 → reassign 剩余 personal task to newSid。handler commit 后 per-id `safeEmit task-changed deleted events`。**Why this option**:caller 不希望 team-bound task 跨 baton 保留(典型 plan 收口后 abandon 中间 task / 不希望 newSid 继承 stale team context)。personal task 仍正常过继给 newSid(skip 仅删 team task);不依赖 caller archive 后 CASCADE — task 在 handoff 时被立即 DELETE 语义干净零中间窗口
  - **`archive_caller=false` 优先级**(沿用 F1 修法):caller 显式 `archive_caller=false` 时 reassign 整段被 skip(caller 仍 active 继续 own 自己 task),`team_task_policy` 不执行 — ok return `taskReassignment={status:'skipped', reason:'archive-caller-false', policy: <resolvedPolicy>}`(policy 字段仍透传 advisory 告诉 caller 传了什么但实际未执行)
  - **失败兜底**: applyHandOffSkipPolicy DB throw / reassignOwner SQL 异常都仅 warn 不阻塞 ok return — task 过继是 nice-to-have,baton 本质是 session 接力;但 caller 通过 ok return `taskReassignment` 字段(`'ok'+count+policy[+policyWarning+unadoptedTeamIds]` / `'failed'+error+policy` / `'skipped'+reason+policy`)看到结果(F3 + v024 §D4)。配合 v023 ON DELETE CASCADE 让 caller archive 后被物理删时 task 已留新 session 名下不被 CASCADE 删
- **ShutdownAllTeammates 不调 reassignTaskOwner**(IPC `AgentDeckTeamShutdownAllTeammates` + hand_off baton-cleanup phase 1):与 hand_off caller→newSid 不对称。teammate 关闭后 task 仍在 teammate 名下,被 `LifecycleScheduler.historyRetentionDays` TTL GC 触发 `sessionRepo.delete` 时 CASCADE 删。**设计取舍**:teammate context 已死 task 本质无主,删干净是合理设计(配合 GC 主语义)。
