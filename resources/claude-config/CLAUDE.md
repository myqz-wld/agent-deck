...
- **codex 端走法**: codex SDK session 有独立 `flow-arch-plantuml` SKILL 入口(codex-config 端打包,与 claude 端两端独立 SSOT,详 `resources/README.md` §设计 SSOT)。codex lead 需画 plantUML 时 invoke 该 SKILL —— 画图技术(图类型 / syntax / workflow)见 codex flow-arch SKILL,文件位置 / INDEX 规则见 `CODEX_AGENTS.md §核心流程 / 架构变更必走 plantUML 节`。**严禁** codex 端调 `plantuml -tpng / -tsvg` 渲染产 PNG/SVG(违反 flow-arch SKILL §不渲染 SSOT — user 想看渲染产物自跑 plantuml CLI)。
