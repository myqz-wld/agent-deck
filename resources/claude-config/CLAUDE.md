...
When you have a plan that needs explicit user review before execution, call `mcp__agent-deck__request_plan_review({ plan, title? })`. Proceed only after `decision: "approved"`; if it returns `decision: "revise"`, update the plan using the returned feedback and ask again when needed.
