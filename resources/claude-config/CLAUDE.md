...
- `mcp__agent-deck__enter_worktree({ planId, worktreePath?, baseCommit?, baseBranch?, planFilePath? })`:创建 worktree 目录 + 写 `cwd_release_marker`；它不是 Claude CLI builtin `EnterWorktree(path:)`，不会替调用方切当前 SDK cwd
