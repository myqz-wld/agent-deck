...
- `spawn_session({ adapter, cwd, prompt, provider?, model?, thinking?, ... })` starts a parallel SDK session and returns `sessionId`, resolved `provider`, `spawnPromptMessageId`, and spawn-limit state. `adapter`, absolute `cwd`, and non-empty `prompt` are required. `adapter` accepts only `claude-code`, `codex-cli`, or `grok-build`. For Claude, `provider` is a Gateway profile id such as `deepseek`; for Codex it is a `model_provider`; Grok rejects it. Suggested models are Claude `haiku` / `sonnet` / `opus` / `fable` or a selected Gateway model id, Codex `gpt-5.6-sol` / `gpt-5.6-terra` / `gpt-5.6-luna` / `gpt-5.5` / `gpt-5.4`, and Grok `grok-4.5`; any non-empty model id is passed through unchanged. Codex thinking is `low` / `medium` / `high` / `xhigh` / `max` / `ultra`; Claude thinking is `low` / `medium` / `high` / `xhigh` / `max`; Grok thinking is `low` / `medium` / `high` / `xhigh`. Provider/model/thinking precedence is explicit argument > Agent runtime/frontmatter > persisted same-adapter source selection > application default > adapter-native default. `grokSandbox` is owned only by `grok-build` and requests the sandbox profile used to start that target's ACP child. Built-ins are `off`, `workspace`, `devbox`, `read-only`, and `strict`; trimmed custom profile names from user or project `sandbox.toml` are also accepted. Omission follows same-adapter inheritance, then the Agent Deck Grok default and Grok-native configuration. Grok managed requirements may override the request, and Grok ACP tool permissions remain separate. If validation fails, retry with the exact value or action in `hint`, or omit the optional override. Passing `teamName` creates or reuses a shared team.
