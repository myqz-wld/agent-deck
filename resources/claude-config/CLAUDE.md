<!-- 由 resources/claude-config/CLAUDE.md 打包注入 SDK system prompt 末尾;维护说明详 resources/README.md。 -->
...
**加载范围**:本文件总是随应用打包注入到 SDK system prompt 末尾。`settingSources: ['user','project','local']` 的交互式 SDK 会话同时加载 user CLAUDE.md;`settingSources: []` 的内部 oneshot（如间歇总结）不加载 user CLAUDE.md。Agent Deck 协议 / plan / prompt 资产维护约定在本文件自闭环生效。
...
---
## 复杂 plan：Agent Deck baseline 最小协议

复杂 plan 用于跨会话、高风险或需要 worktree 隔离的工作。本节只保留 Agent Deck 必须自带的 plan / worktree / handoff / cleanup 协议；通用 RFC、spike、planning 方法论不在 baseline 展开，用户 skill 只能作为增强层。

### 触发

- 预计跨多个会话、多个 phase 或多模块才能收口。
- 改动破坏性强、实验性强，或失败时需要整片回退。
- 改动跨 adapter、schema、进程、权限或关键 SDK 行为边界。
- 当前会话准备 hand off，且后续 agent 必须靠文件恢复进度。

### Plan 文件

触发后先写 plan，再进 worktree。plan 必须用绝对路径，合法位置：

- `<main-repo>/ref/plans/<plan-id>.md`：completed 后入 git 的归档位置。
- `<main-repo>/.claude/plans/<plan-id>.md`：project-local in_progress 草稿，完成后归档到 `ref/plans/`。
- `~/.claude/plans/<plan-id>.md`：跨项目 plan 或 CLI `/plan` 默认位置。

`<plan-id>` 用 `<topic>-<YYYYMMDD>`，字符集限 `[A-Za-z0-9._-]`，单 segment ≤ 64 字符。frontmatter 必含 `plan_id` / `created_at` / `worktree_path` / `status: in_progress|completed|abandoned` / `base_commit` / `base_branch`；frontmatter 用 snake_case，MCP tool args 用 camelCase。

正文必须包含：总目标与不变量、设计决策、步骤 checklist、当前进度、下一会话第一步、已知风险。`base_branch` 是切 worktree 时主仓库所在分支，完成时合回它，不是默认合 main。

### Review Gate

复杂 plan 写完、进 worktree 前，按可用 review 流程评审 plan。Agent Deck deep-review 可用时评审 `{kind: 'plan', paths: ['<plan-abs-path>']}`；无 deep-review 时走 simple-review；MCP / review skill 都不可用时交给用户人审。CRITICAL/HIGH finding 未处置前不进 worktree。
...
- plan 文件是跨会话设计与进度文档；MCP task store 是单会话 / 跨会话任务可见性通道。完成后的 changelog / review 记录按当前项目 skill 组织，Agent Deck baseline 不规定产物布局。
- 复杂 plan 的 worktree 隔离是机械执行边界；架构、选型、breaking change 仍要走合适的设计对齐或 review 流程，结论写回 plan「设计决策」。

## 提示词资产维护

**改长生命周期 prompt 资产前必走本节**——即 `resources/claude-config/CLAUDE.md`（本文件）/ `resources/codex-config/CODEX_AGENTS.md` / `agent-deck-plugin/agents/reviewer-{claude,codex}.md` / `agent-deck-plugin/skills/*/SKILL.md` / 注入 SDK 的 mcp tool description。**不适用**：src/ 业务代码注释 / 一次性 prompt（spawn 临时拼的 reviewer / cold-start prompt）/ 历史快照（ref/ 下写定不改）。
...
2. **只写当前事实**：禁「兼容旧 X / 旧结构 / 历史升级 / 迁移计划 / 过渡期 / 老版本仍可用 / 未来可能 / TODO / FUTURE / 后续会加」；废弃的功能 / 字段直接删，不留 deprecated 注释。不要保留“目录建立之前 / 旧路径仍支持 / 后续迁移”这类兼容逻辑；旧行为若仍影响当前 agent 动作，改写成当前可执行规则，不讲历史。
...
2. **当前事实**：`grep -nE '兼容|FUTURE|TODO|未来|向后|deprecated|过渡期|后续会加|老版本|旧结构|历史升级|目录建立之前|迁移' <file>` ≥ 1 → 删（meta 引用 / 行为锚点除外）
