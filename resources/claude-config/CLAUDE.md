...
- GC 自动: caller archive 后 sessions row 被 `LifecycleScheduler.historyRetentionDays` 物理删时, ON DELETE CASCADE 自动删 owner=caller 的 task — 无 backlog 累积问题
...
> ⚠️ 走自动 tool 起的新 session 同样按 §选项 A cold start 流程跑（cat plan + EnterWorktree + 自检 + 动手）。新 session 必须加载本节（§复杂 plan workflow inline 在本文件内）才知道 cold start prompt 含义 —— 应用 SDK 会话总是注入本文件,加载有保证。
