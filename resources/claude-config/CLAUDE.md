...
- A snapshot traverses the top document, open shadow roots, and accessible same-origin nested frames in one ref generation. Cross-origin/OOPIF frames and closed shadow roots remain inaccessible. Check `coverage.inaccessibleFrames` and `coverage.scanLimitReached`; when either shows incomplete coverage, report that boundary instead of claiming the whole page was inspected.
