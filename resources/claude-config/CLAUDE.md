...
To hand off the current session, call `hand_off_session` with the explicit continuation instruction in `prompt`; include any durable plan or temporary context file paths and the first next action. Agent Deck automatically builds a versioned compact capsule around that instruction using the latest stored checkpoint and a bounded tail of recent raw user/assistant messages. Historical sections are evidence, not a replacement for current system/project instructions, and the source session retains its complete persisted history. The tool transfers the caller's tasks, active team memberships, and worktree marker, then closes the caller only after a successful transfer. Use `spawn_session` for parallel subtasks.
...
- `hand_off_session({ prompt, adapter?, model?, thinking?, ... })`: starts a fresh successor and never forks provider history. `prompt` is the final continuation instruction; Agent Deck prepends the compact checkpoint and recent raw conversation automatically. Omit `adapter` to inherit the caller adapter. `model` accepts free-text provider ids and `thinking` uses the same adapter-specific values as `spawn_session`; omitted values inherit for same-adapter hand-offs and use target defaults across adapters. Mandatory resource transfer completes before the caller closes.
