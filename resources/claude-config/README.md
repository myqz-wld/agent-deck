...
- 改 `agent-deck-plugin/skills/*/SKILL.md` → 只改本目录下的 claude-config SSOT，再跑 `node scripts/sync-codex-skills.mjs` 同步到 `resources/codex-config/agent-deck-plugin/skills/` 镜像；codex 侧 skill 镜像是生成物，不手改
...
- `resources/claude-config/agent-deck-plugin/agents/reviewer-claude.md` / `resources/codex-config/agent-deck-plugin/agents/reviewer-codex.md` —— `deep-review` SKILL 用的两 reviewer teammate body（**两端独立 SSOT**，sync-codex-skills.mjs 不同步 agents/）
- `skills/deep-review/SKILL.md` —— 多轮异构 review × fix 收口的 SKILL（claude-config 单源，codex-config 镜像）
- `skills/flow-arch-plantuml/SKILL.md` —— 核心流程 / 架构变更画 plantUML 的 SKILL（仅 claude 端）
- `skills/hello-from-deck/SKILL.md` —— plugin 自检 SKILL（claude-config 单源，codex-config 镜像）
