...
> **前提**：会话挂了 agent-deck-mcp（应用 Settings → Agent Deck MCP server 已启用）。本 SKILL 完全走 `mcp__agent_deck__*` 5 个 tool（spawn_session / send_message / wait_reply / list_sessions / shutdown_session）编排。
