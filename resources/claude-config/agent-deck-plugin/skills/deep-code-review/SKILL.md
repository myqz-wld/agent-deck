---
name: deep-code-review
description: 深度 code review — 多轮异构 reviewer 对抗 + 三态裁决，挖深层 bug（race / leak / 边角 / 架构 / 安全 / 测试盲区）。前提：会话挂了 agent-deck-mcp。触发：「深度 code review」/「deep code review」/「双对抗 review」/「review fix 多轮」/「再 review 一轮」。
---

# Deep Code Review — 多轮异构对抗 review × fix 收口

把「review → fix → review → fix → ... 直到挖不出新问题」封装成可复用流程。重点是**多轮挖深**：第 1 轮抓浅层（typo / null / 错变量），第 2-3 轮挖深层（race / leak / 边角 / 架构 / 测试盲区 / 性能尾延迟）。

> **前提**：会话挂了 agent-deck-mcp（应用 Settings → Agent Deck MCP server 已启用）。本 SKILL 走 `mcp__agent-deck__*` tool 编排（核心 5-6 个：`spawn_session` / `send_message` / `reply_message` / `check_reply` / `shutdown_session`，必要时叠加 `wait_reply`；救火 4-5 个：`list_sessions(spawned_by_filter)` / `get_session` / `archive_plan` / `hand_off_session`（CHANGELOG_99 改名前 `start_next_session`；CHANGELOG_99 双模式:plan-driven / generic），跨会话 hand off / lead context 重置后捡回 stranded reviewer 用）。Backend 协议（spawn 返回 `spawnPromptMessageId` / **check_reply 非阻塞 poll** 按 messageId 锚点 / wire format / shutdown_session 不删数据）SSOT 在应用 CLAUDE.md「Agent Deck Universal Team Backend」节。

> ⚠️ **CHANGELOG_99 已知缺陷(过渡期)**: `reply_message` 当 caller 是 lead 给 teammate 发 reply 时**不触达**(J fix 一刀切拦截了所有 `replyToMessageId != null` 的 message 不 dispatch)。**短期绕过**:lead 给 teammate 发新 message + 想链接 reply chain → 改用 `mcp__agent-deck__send_message({session_id, team_id, text, reply_to_message_id})` 显式传字段。teammate reply lead 仍用 `reply_message`(target = sender 路径正是 J fix 设计场景,正常工作)。**lead 等 reply 默认走 `check_reply` 非阻塞 poll,严禁 `wait_reply` 阻塞 lead 上下文**(阻塞期间 lead 无法响应 user / 处理其他 input)。**完整修法**见新 plan `mcp-tool-simplify-20260514`(完全删 reply_message + wait_reply + check_reply + J fix,统一 send_message + adapter dispatch 心智模型)。

## 何时用

- 关键路径 / 核心抽象的代码变更（multi-client / 并发 / lifecycle / 资源管理）
- 跨多模块、影响主链路（≥ 200 行 / ≥ 5 文件）
- MR 提交前最后一道闸门
- **不适合**：trivial 改动（typo / 单点 rename / 显然措辞修订）— 一轮人审就够

## 核心设计

### 异构对抗

每轮**必须**两个 reviewer 同时起；初轮 spawn，**后续轮次复用同一对**（send_message 不重新 spawn）：

| Reviewer A | Reviewer B |
|---|---|
| `reviewer-claude` teammate（claude-code adapter，Opus 4.7 default thinking） | `reviewer-codex` teammate（claude-code adapter wrapper，内部 Bash 跑外部 codex CLI gpt-5.5 xhigh） |

两个 teammate 完全独立（互不知道对方存在）。**lead 自己**做三态裁决，不让 teammate 既当 reviewer 又当裁判。同源化禁令（不可降级到双 Claude）见 §失败兜底 引用。

### 多轮挖深

| 轮次 | focus | 期待 finding |
|---|---|---|
| Round 1 | 修复正确性 / 是否引新问题 / 测试质量 | 浅层 bug、API 误用、明显 regression |
| Round 2 | 边界条件 / 并发 race / 资源 lifecycle | race window、cleanup 漏 path、状态机边角 |
| Round 3 | 架构耦合 / 安全 / 性能尾延迟 | 跨模块隐患、信任边界破坏、p95/p99 异常 |
| Round 4+ | 上轮残留 + 用户特别关注 | 收口或拒合 |

### 三态裁决

- ✅ **真问题**：双方独立提出 / 一方提出且现场实践验证成立（写 test 复现 / grep 调用点 / 读真实代码）→ 必修
- ❌ **反驳**：被对抗或现场核实证伪 → 不修，记反驳依据
- ❓ **部分 / 未验证**：双方角度不同 / 一方提出但纯文本推理（含弱断言）尚未实践验证 → 综合后决定；未验证强制降级非 HIGH

每条**单方独有**：HIGH 候选 → §Step 4 反驳轮；MED → lead 自己 Grep / Read 现场验证；LOW/INFO → 直接列 ❓。**不接受没验证的 ✅ HIGH**。

### 收口 / 拒合

**收口**（全部满足）：双 reviewer 都「可合」+ 0 个 HIGH/MED + 上轮真问题已 fix 通过测试。
**拒合**：还有 HIGH 未修 / 双方仍发现 ≥ 5 条新真问题 / 用户主动停。

## 执行模板（7 步）

| Step | 动作 | 关键字段 / 等什么 |
|---|---|---|
| 0 | 准备 `cwd`（仓库 / worktree 绝对路径）+ `scope`（从 git diff / 文件清单确定，路径与 cwd 同前缀）| — |
| 1 | 并发 spawn 两 reviewer：`spawn_session({adapter:'claude-code', cwd, prompt, team_name, agent_name, display_name})` × 2，`agent_name='reviewer-claude'` / `'reviewer-codex'`（D1 实施，body 自动注入到 prompt 头）；可选 `display_name`（如 `'reviewer-claude · batch A'`）让 SessionList / TeamDetail / wire prefix 一眼区分多组并行 review，不传则 fallback 到 agent_name | 各拿 `spawnPromptMessageId`，是首轮 reply 锚点；两 spawn 之间不要等 reply |
| 2 | **非阻塞 user-driven poll**：lead 调 `check_reply({message_id: spawnPromptMessageId})` × 两个 reviewer 各一次，立即返回告诉 user「已派 2 个 reviewer 跑 review，UI 实时显示进度，reply 来了或 reviewer 卡了请告诉我；期间你可以随时插话（跳过 X / 优先看 Y / abort 某 reviewer）」。**之后每次 user 发 message 时，lead 在响应前先 check_reply 一遍**两个 reviewer：两个都收到 reply → 进 Step 3；都没 reply → 处理 user message + 决定是否 nudge teammate（详 §lead 必须自己 nudge 兜底）。**严禁阻塞型 timer**：`wait_reply`（30min 阻塞 user input）/ `ScheduleWakeup`（仅 /loop dynamic mode 适用）/ Bash sleep（阻塞 lead 整个 turn）；**非阻塞 timer 兜底允许**（详 §timer fallback：`CronCreate(recurring: true, durable: false)` 周期 fire prompt 让 lead 自己 wake check_reply，user 离开期间也能推进） | 拿两份独立 finding 列表（user-driven 触发 + timer 兜底，开销零） |
| 3 | 三态裁决：双方一致 → ✅；单方独有 + HIGH → Step 4 反驳轮；单方独有 + MED → lead 自己 Grep/Read 验证；单方独有 + LOW/INFO → 直接列 ❓ | — |
| 4 | 反驳轮（仅 HIGH 单方独有）：`send_message` 把 A 的 finding 给 B 反驳（reverse 同理）→ **同 Step 2 一样 user-driven poll**：调一次 `check_reply({message_id: <send 返回的 messageId>})` 后立即返回告诉 user，下次 user 发 message 时再 check 反驳 reply | 同一条 finding 只反驳一次；反驳后仍不能定 → lead 自己验证；还不行 → 降级非 HIGH |
| 5 | fix → 下一轮：lead 改代码后，**复用同一对** teammate 发 Round 2 prompt（必带 `skip` 字段 = 上轮 ✅ fix 摘要，每条按格式 `已修：<filepath:line> <一句话改动> (commit <hash>)`，避免 reviewer 重复列）→ user-driven check_reply poll → 回到 Step 3 直到收口 | — |
| 6 | 收尾：`shutdown_session` × 2 | shutdown 不删 events / messages，lead 仍可在裁决报告里引用。**想几小时后再 R3 复用 reviewer mental model 时不要 shutdown** —— 留着让 lifecycle scheduler 自然 dormant，下次 send_message 会自动 SDK resume 复原对话历史；只有彻底不再用本对 reviewer 才 shutdown（详 §dormant ≠ 丢 mental model 反直觉） |

**为什么 user-driven check_reply 主路径 + 非阻塞 timer 兜底，而非 wait_reply 阻塞**（设计取舍，**不要被本能反带回 wait_reply / 阻塞型 timer**）：

- **lead 必须保持对 user 的响应能力**：deep review 跑 30min 是常态，user 看 UI 实时活动可能随时想：「跳过 X 这条 finding 我已经判定不修」/「重点看 race 那条」/「reviewer-codex 那边卡了，先 abort」。wait_reply 期间 lead 完全聋，user 反馈被 SDK input queue 排队到 wait_reply 超时才处理 → 体感「打字了但 agent 没反应」（CHANGELOG_95 反馈来源）
- **user-driven 是主路径，timer 是兜底叠加（非阻塞）**：lead 每次 turn 开头先调 `check_reply` 一遍处理可能已到的 reply，剩余时间响应 user message。但 user 可能离开应用看其他东西（吃饭 / 处理别的事） → reviewer reply 来了没人触发 lead poll → 进度滞涨。**兜底**：lead spawn 后用 `CronCreate(recurring: true, durable: false)` 起一个周期性自我唤醒（fire 一条 prompt 让 lead idle 时自己 check_reply 一遍），user 在场不在场都不卡。`durable: false` = session-only 不写盘，lead context 重置 / shutdown 自动消亡。**严禁阻塞型 timer**：ScheduleWakeup 限 `/loop dynamic mode`（本场景非 /loop，工具规范明禁）；Bash sleep 阻塞 lead 整个 turn 期间无法响应任何 user message；wait_reply 30min 阻塞同理
- **设计依据**：应用 CLAUDE.md `§check_reply` 节明示「lead 自己控 poll 节奏」—— 「自己控」可以是 user-driven，也可以是非阻塞 CronCreate timer。两者叠加：user 在场时 user-driven 触发更快（reply 来了就 ping），user 离场时 CronCreate 兜底防滞涨

### timer fallback（CronCreate 非阻塞自我唤醒）

**触发**：Step 1 spawn 完成后，**与 Step 2 同 message** 起一个周期性 cron 任务作 user-driven 兜底：

```ts
const job = await CronCreate({
  cron: '7,17,27,37,47,57 * * * *',  // 周期 10min（避开 :00/:30 减少 fleet 撞峰，详 ScheduleWakeup 工具描述同款规则）
  prompt: '[deep-code-review timer] 检查 reviewer-claude (sid=<A>) 与 reviewer-codex (sid=<B>) 在 team <T> 内最近一轮 message_id=<originalA>/<nudgeA?> 与 <originalB>/<nudgeB?> 是否有 reply；有就进入 Step 3 三态裁决；没有就 get_session 看 lastEventAt 决定是否 nudge / abort',
  recurring: true,
  durable: false,                     // session-only，lead context 重置 / shutdown 自动消亡
});
// 拿到 job.id 暂存，下面三个时机调 CronDelete
```

**清理时机**（任一命中立即 `CronDelete(job.id)`）：
- 收到双 reply 进 Step 3 三态裁决
- 进入 Step 5 fix 阶段（开始改代码 → 旧 messageId 已不再是 wait 锚点，timer 拿旧 id 永远 null 浪费 turn）
- Step 6 `shutdown_session` × 2 收尾
- 用户主动 abort SKILL（user 说「停 / 不再 review」）

**周期选 7-10min 而非 1-2min** 的理由：
- reviewer 跑 review 一轮典型 5-15min，1-2min 周期会触发大量空 poll 浪费 lead context
- 7,17,27,37,47,57 错峰避开 :00/:30 在多 reviewer 场景同款 fleet 错峰收益（不同 lead 实例 cron 不同时刻撞峰）
- 重 review 场景（多 batch 并发 / 文件 ≥ 30）可视情况放宽到 15-20min（cron 改 `13,33,53 * * * *`）

**为什么不用 ScheduleWakeup**：工具规范明示「Schedule when to resume work in /loop dynamic mode」—— 仅当**用户已经 invoke /loop** 时适用，本 SKILL 是普通 conversational 流程不是 /loop，misuse 工具规范。CronCreate `recurring: true, durable: false` 是当前 SDK 唯一合规的 in-session 非阻塞 timer 选项。

**timer 与 user-driven 关系**：两个机制叠加同一份 check_reply 调用。user 在场 ping → user-driven 触发；user 不在场 / cron fire → timer 触发。lead 收到「[deep-code-review timer]」prompt 后按 §lead 必须自己 nudge 兜底 走同款 check_reply 两 messageId / 必要时 nudge / 必要时 get_session 检查 lastEventAt 流程。

### lead 必须自己 nudge 兜底（check_reply 不会自动催 reply）

`check_reply` 是纯非阻塞 poll，不像 `wait_reply` 带 `nudge_text + nudge_after_ms` 自动催 reply 字段。如果 teammate 忘了调 `reply_message`（context 满了忘协议 / 卡审批不响应 / 任何形态的「卡死但还活着」），lead 单纯 poll 永远拿不到 reply。

**触发**：user 主动 ping「reviewer 卡了吗」/「进度？」时，lead 顺便判断是否需要 nudge：

- **轻量 nudge**：lead `send_message(teammateSid, '📍 nudge: 我在等你 reply 上一条 review request；完成后请 reply_message 回我；进度需要更多时间也请回一句告知')`，**记下新 send 返回的 `nudgeMessageId`**

  ⚠️ **关键陷阱（reviewer-codex MED2 实测）**：watcher 会给 nudge body 自动注入 `[from <name> @ <adapter>][msg <nudgeId>]\n` wire prefix（`universal-message-watcher.ts:189-199`）。按 `reviewer-{claude,codex}.md` 协议 teammate 强制 regex 抓收到消息**第一个** `[msg ...]` 当 reply_to_message_id → teammate 会 reply nudgeId 而非原 originalId → lead 仅 poll originalId 永远拿不到 reply。

  **修法**：lead 之后**同时 poll 两个 messageId**（伪代码）：
  ```
  const r1 = await check_reply({message_id: originalId});
  if (r1.reply) return r1.reply;            // teammate 真按 original 回
  if (nudgeMessageId) {
    const r2 = await check_reply({message_id: nudgeMessageId});
    if (r2.reply) return r2.reply;          // teammate 按 nudge 回（默认行为）
  }
  // 仍 null → 等下次 user message 触发 poll
  ```
  下一轮（Step 5）lead 用 `send_message` 发新 prompt 时，新 messageId 重新成为「当前 wait 锚点」，旧 originalId / nudgeMessageId 自动作废

- **重 nudge / 升级**（user 连发 ≥ 2-3 次「进度？」+ 两 messageId 都 null + `get_session(teammateSid).lastEventAt` 没动 ≥ 15min）：调 `get_session(teammateSid).lastEventAt` 看 teammate 是否还在动 → 在动 → 告诉 user「reviewer 还在跑只是慢，再等等」；不动 → 走 §失败兜底「`check_reply` 持续返回 `{reply: null}`」recipe（PendingTab 真人介入 / shutdown 重 spawn / 合规兜底）

- **绝不无限 poll**：30min（按 `lastEventAt` 判定 + user 多次 ping 仍无 reply）后 teammate 仍卡死必须 abort 该 reviewer，不要让 lead 在 user 多次 ping 中持续消耗 context

**Prompt 内容约定**（每次 spawn 或 send_message 的 prompt 必带）：
- `output_mode: full_review` 或 `rebuttal`
- `scope`：文件清单（**绝对路径**，与 spawn cwd 同前缀；worktree 内必须含 `.claude/worktrees/<plan-id>/` 前缀，否则 reviewer 会报 `⚠ SCOPE PATH MISMATCH` abort）
- `focus`：本轮重点维度
- `skip`：上轮 ✅ fix 摘要 / 已审过的稳定项；每条按格式 `已修：<filepath:line> <一句话改动> (commit <hash>)`（例：`已修：src/foo/bar.ts:42 加 try/catch 兜 promise reject (commit 438a613)`）
- 输出格式约束（详 reviewer-{claude,codex}.md「输入识别」节）

## 强制约束（lead spot-check 用）

reviewer-* agent body 已强约束 finding 输出契约（文件:行号 + 代码片段 + 验证手段；弱断言只允许出现在 *未验证* 条目里）。lead 收到 reply 后只需抽查：

- 缺文件:行号 / 缺验证手段 → 降 ❓
- 「✅ HIGH」纯文本推理无验证 → 强制降 ❓ 或走反驳轮

## 失败兜底

| 场景 | 处理 |
|---|---|
| reviewer-codex 报失败模板（CLI 不可用 / OAuth 过期 / Bash 卡审批被拒 / timeout）| 通知用户决策（等恢复 / 单方 reviewer-claude 出结论 / 稍后重试 / abort）。**严禁**降级同源双 Claude（同源化破坏异构）。**合规兜底**（仍异构）：lead 自己 Bash `run_in_background: true` + `timeout: 600000` 起外部 codex CLI（按 reviewer-codex.md §codex CLI 调用模板填模板，lead 自己执行而非 wrapper teammate），与 reviewer-claude teammate 仍构成 gpt-5.5 vs Opus 4.7 异构对；reviewer-claude teammate 不动继续按 SKILL 流程跑；lead 拿到独立 codex stdout 后照常做三态裁决。**严禁**让 reviewer-claude teammate 跑一份「codex 视角」补缺（仍是同源化）|
| reviewer-* 报「⚠ FRESH SESSION — in-memory state empty」信号 | teammate 被 SDK 自动重启过，in-memory state 全丢。`shutdown_session` 该 teammate → 重 spawn → 按当前 scope 发 **Round 1 init prompt 全量重跑**（不要继续 Round N+1）|
| reviewer-* 报「⚠ SCOPE PATH MISMATCH」信号 | scope 路径前缀与 spawn cwd 不一致（典型：worktree 场景下 scope 写成主仓库根级形态）。修 scope 路径 → shutdown + 重 spawn + 重发 prompt |
| `check_reply` 持续返回 `{reply: null}`（user 多次 ping 仍无 reply + nudge 后两个 messageId 都 null）| 调 `get_session(teammateSid).lastEventAt` 检查 teammate 是否仍推进：是 → 告诉 user「reviewer 还在跑只是慢，再等等」继续 user-driven poll；否 → teammate 卡审批 / 卡死，提示真人去 PendingTab 处理或走上面合规兜底 |

> 其余 mcp tool error（no-shared-team / ambiguous-team / rate-limit / 投递 failed / 跨会话捡 stranded reviewer）走 mcp tool schema 自描述错误处理；高级救火场景见应用 CLAUDE.md「Agent Deck Universal Team Backend」节。

## dormant ≠ 丢 mental model（反直觉，**别再推理错**）

reviewer 跑完一轮 review 后 idle 一段时间被 lifecycle scheduler 自动转 `dormant`（应用 CLAUDE.md「shutdown 不删数据」节有同款 callout）。**dormant 不丢 in-memory mental model**：

1. lifecycle scheduler 转 dormant 只 `setLifecycle('dormant')` + abort SDK live query + 清 in-process Map，**不删 jsonl 文件**
2. 下次 lead 调 `send_message` 给 dormant reviewer：universal-message-watcher → adapter.sendMessage → sdk-bridge 检测 `!sessions.has(sid)` → recoverAndSend 预检 jsonl 在 → `createSession({resume: oldSid, prompt})` → CLI 复原对话历史 → reviewer 看到自己上轮 reply + 已读文件痕迹 → mental model 通过 conversation history 隐式保留 ✅
3. 反之**只有 jsonl 缺失**（典型：用户手动删 ~/.claude/projects 目录 / 应用重装 / 跨设备同步未带 jsonl）走 hard fail fallback (createSession 不带 resume) 才真 fresh = reviewer 触发 §失败兜底 表里的 `⚠ FRESH SESSION` warn → 这时才必须 shutdown + 重 spawn

**反直觉提示**：lead 看到 reviewer 是 dormant **不要**等价为「fresh session 必须重 spawn」。dormant 之间的差距是 SDK live query 是否 active 而非 conversation 是否丢。**保留 reviewer 跨轮 mental model 复用就别 shutdown**，让它们自然 dormant；下次 send_message 自动 resume。具体机制详 `src/main/adapters/claude-code/sdk-bridge/recoverer.ts:103-220` 与 user CLAUDE.md「会话恢复 / 断连 UX」节。

## 与决策对抗节的关系

本 SKILL = **多轮**深度 review × fix × 反驳轮编排（teammate 模式，跨轮 context 持久化、反驳轮被反驳方有自身上轮推理链）。**单次决策对抗**（单点判定 / plan 评审 / 约定升级）走 `~/.claude/CLAUDE.md`「决策对抗 → 主路径」节**双 Bash 直接起外部 CLI** 即可，零 SDK 状态。两个场景两个姿势，不混用。
