...
description: 深度 code review — 多轮异构 reviewer 对抗 + 三态裁决，挖深层 bug（race / leak / 边角 / 架构 / 安全 / 测试盲区）。前提：会话已挂载 agent-deck-mcp。触发：「深度 code review」/「deep code review」/「双对抗 review」/「review fix 多轮」/「再 review 一轮」。
---
...
| 2 | **告诉 user**「已派 2 个 reviewer 跑 review，UI 实时显示进度，reply 来了我会自动收到处理；期间你可以随时插话（跳过 X / 优先看 Y / abort 某 reviewer）」**然后等 reply 自动注入**。reviewer 跑完调 `send_message + reply_to_message_id` 后 reply 自动注入 lead conversation flow（dispatch 机制 / wire prefix 详应用 CLAUDE.md §Universal Team Backend）。**lead 不主动 poll** — UI 上 reply chip ↩ 标识让用户知道这条 user message 是 reviewer 注入而非自己输入 | 两份独立 finding 自动到达 lead conversation；user 也可在 UI 实时看 |
