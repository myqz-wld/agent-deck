...
> **前提**：会话挂了 agent-deck-mcp（应用 Settings → Agent Deck MCP server 已启用）。本 SKILL 完全走 `mcp__agent_deck__*` 10 个 tool 编排。Backend 协议（spawn 返回 `spawnPromptMessageId` / **check_reply 非阻塞 poll** 按 messageId 锚点 / wire format / shutdown_session 不删数据）SSOT 在应用 CLAUDE.md「Agent Deck Universal Team Backend」节。
