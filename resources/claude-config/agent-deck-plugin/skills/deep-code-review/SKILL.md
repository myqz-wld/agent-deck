...
| 1 | 并发 spawn 两 reviewer：`spawn_session({adapter:'claude-code', cwd, prompt, team_name, agent_name})` × 2，`agent_name='reviewer-claude'` / `'reviewer-codex'`（D1 已实施，body 自动注入到 prompt 头）| 各拿 `spawnPromptMessageId`，是首轮 reply 锚点；两 spawn 之间不要等 reply |
| 2 | `Promise.all` 两个 `wait_reply({message_id: spawnPromptMessageId, timeout_ms: 600_000})` | 拿两份独立 finding 列表（10 min 给重 review 足时间）|
...
| `wait_reply` 超时（默认 600_000ms）| `timed_out=true`。检查 `get_session().lastEventAt` 是否仍在推进：是 → 加大 timeout 重 wait；否 → teammate 卡审批 / 卡死，提示真人去 PendingTab 处理或走上面合规兜底 |
