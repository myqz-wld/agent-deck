...
| 整轮收口 | §Step 6 cleanup teammate 之前 | `task_list({status_filter: "active"})` 应返 0 条；若有遗留逐条 `task_update(status:"abandoned", description:"收口前 abort：<原因>")` |
...
#### Step 2.2 — 同一 message 并发起两个 teammate

调 **两次 `Agent` 工具**（与 §Fallback 用的是同一个 Task tool，但参数列表不同），**必须四个字段齐全**：

```text
Agent 1:
  subagent_type: "agent-deck:reviewer-claude"
  name:          "reviewer-claude"               # 不传或重名 → 起不来
  team_name:     "<上面 Step 2.1 的 team_name>"   # ❗ 不传退化成纯 subagent
...
- lead 等待期间不要启动其他重活
- spawn 完 + 等待之前调一次 `mcp__tasks__task_create` 建 `R_1`（参 §Milestone tracking 表第 1 行）

#### 自检

如果 `TeamCreate` 工具不存在（CLI 老版本 / agentTeamsEnabled OFF / 非 SDK 会话）→ 退到 §Fallback。**不要试图自己用 `Agent(subagent_type=..., prompt=...)` 单参数起 reviewer**——那等同于 §Fallback subagent 模式，reviewer-codex 内部 Bash 必挂（详见 §Fallback 红字警告）。
