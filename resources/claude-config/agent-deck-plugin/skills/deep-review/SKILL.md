...
5. **生成 manifest**：`<reviewRoot>/.deep-review-cache/<invocationId>/manifest.json`(放 invocation 子目录内,与 cache files 同级)
   ...
   ```
6. **SKILL 启动 step 0 sweep 旧 orphan**：扫 `<reviewRoot>/.deep-review-cache/*/manifest.json`,`createdAt` 距今 > 24h 的 invocation 子目录全删 `rm -rf <invocation-id>/`(中断的 SKILL 留下 orphan 不会无限累积)
