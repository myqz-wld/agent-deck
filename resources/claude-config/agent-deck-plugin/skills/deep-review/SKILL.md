...
**reviewRoot 定义**：`reviewRoot = Step 0 的 cwd`。cwd 可以是普通 repo root，也可以是 plan worktree root；本节不要求一定处在 worktree。cache 一律落在 `<reviewRoot>/.deep-review-cache/<invocationId>/`。
