...
6. **SKILL 启动 step 0 双自检**：
   ...
   - **`.gitignore` 自检**：`Bash: grep -q '^\.deep-review-cache/' <reviewRoot>/.gitignore` 失败且 `ack_cache_unignored` 不是 true → warn caller「`.gitignore` 缺 `.deep-review-cache/` entry，cache 文件可能被 commit；请加 entry 或接受风险继续」并 stop for explicit consent。批处理 / 自动调度场景必须传 `ack_cache_unignored: true` 才能跳过自检并接受 cache untracked 风险。
...
> **Lead turn boundary**：Step 2 / Step 4 / Step 5 发出 reviewer 请求后，告诉 user 当前状态并停止当前 turn；reviewer reply 会作为后续 user-role message 注入，届时继续 Step 3 裁决。不要用 `sleep` / `get_session` 循环在同一 turn 等 reply。
...
| 2 | **告诉 user**「已派 2 个 reviewer 跑 review，UI 实时显示进度，reply 来了会作为后续 user message 注入；我收到后继续裁决。期间你可以随时插话（跳过 X / 优先看 Y / abort 某 reviewer）」**然后停止当前 turn**。reviewer 跑完调 `send_message + replyToMessageId` 后 reply 自动注入 lead conversation flow（dispatch 机制 / wire prefix 详应用约定文件 §Universal Team Backend）。**lead 不主动 poll** | 两份独立 finding 后续注入；user 也可在 UI 实时看 |
| 3 | 三态裁决：双方一致 → ✅；任一 CRITICAL/HIGH → Step 4 反驳轮；单方独有 MEDIUM → lead 自己 Grep/Read 验证；LOW/INFO → 直接列 ❓ | — |
| 4 | 反驳轮（所有 CRITICAL/HIGH）：`send_message({sessionId: B-sid, teamId, text: '<A 的 finding 全文> 请独立反驳', replyToMessageId: <Round 1 messageId>})` 把 A 的 finding 给 B 反驳（reverse 同理）→ 停止当前 turn；B 反驳 reply 后续注入 | 同一条 finding 必须记录支持论与反驳论；反驳后仍不能定 → lead 自己验证；还不行 → 降为 MEDIUM 或更低 |
| 5 | fix → 下一轮：lead 修 CRITICAL/HIGH 真问题；MEDIUM 明确「本轮修 / 接受风险 / 记 follow-up」；改代码 / 改 plan 后，**复用同一对** teammate 调 `send_message` 发 Round 2 prompt（必带 `skip` 字段 = 上轮 ✅ fix 摘要，每条按格式 `已修：<filepath:line> <一句话改动> (commit <hash>)`，避免 reviewer 重复列）→ 停止当前 turn；reply 后续注入后回到 Step 3 → ... → 直到 §收口 判定满足（0 CRITICAL/HIGH + 双方共识可合）才进 Step 6。**多轮迭代期间绝不 shutdown**（违反「复用同一对 teammate」invariant — 重 spawn 会丢跨轮 mental model + 撞 reviewer-codex FRESH SESSION 拒） | — |
...
> **lead 自然推进**：reply 走 adapter dispatch 自动注入 receiver SDK conversation flow → reviewer reply 到 lead 后成为后续 user-role message → lead 当作普通 user input 处理 → 继续裁决 / 进入下一步。
...
**lead 自检步骤**：
1. 仅在 user 下一轮询问状态或达到 30min 阈值时调 `get_session(reviewerSid).lastEventAt`，看 reviewer 是否还在推进（recent ts → 还在跑只是慢，告诉 user 再等等并停止当前 turn；非 recent → 卡死）
...
3. nudge 后停止当前 turn；下一轮仍无 reply 且达到阈值 → 走 §失败兜底「reviewer 持续卡死」recipe（PendingTab 真人介入 / shutdown 重 spawn / 合规兜底）
...
| reviewer 持续不 reply（user 多次 ping 仍无 reply + lead nudge 后仍无 reply）| 仅在 user 下一轮询问状态或达到 30min 阈值时调 `get_session(reviewerSid).lastEventAt` 检查 reviewer 是否仍推进：是 → 告诉 user「reviewer 还在跑只是慢，再等等」并停止当前 turn；否 → reviewer 卡审批 / 卡死，提示真人去 PendingTab 处理或走上面合规兜底 |
