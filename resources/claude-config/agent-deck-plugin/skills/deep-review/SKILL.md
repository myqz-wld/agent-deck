...
> 完整契约 SSOT：`~/.claude/CLAUDE.md` §决策对抗 §三态裁决 + §Finding 输出契约。
...
> 完整契约 SSOT：`~/.claude/CLAUDE.md` §Finding 输出契约。

## 失败兜底

| 场景 | 处理 |
|---|---|
| reviewer-codex 报失败模板（codex SDK 起不来 / OAuth 过期 / shell tool call cancel / sandbox 拒 / timeout / codex thread jsonl 缺失 fresh-session abort）| 通知用户决策（等恢复 / 单方 reviewer-claude 出结论 / 稍后重试 / abort）。**严禁**降级同源双 Claude（同源化破坏异构）。**合规兜底**（仍异构）：lead 自己用当前 adapter 的 Bash/shell 起外部 codex CLI（按 user 全局模板 `~/.claude/templates/reviewer-codex.sh.tmpl` 填；Claude lead 用 Bash `run_in_background: true` + `timeout: 600000`，Codex lead 用 shell 后台/长超时等价能力），与 reviewer-claude teammate 仍构成 gpt-5.5 vs Opus 4.7 异构对；reviewer-claude teammate 不动继续按 SKILL 流程跑；lead 拿到独立 codex stdout 后照常做三态裁决。**严禁**让 reviewer-claude teammate 跑一份「codex 视角」补缺（仍是同源化）|
| reviewer-claude 报失败模板（claude SDK 起不来 / OAuth 过期 / sandbox 拒 / timeout / claude jsonl 缺失 fresh-session abort）| 通知用户决策（等恢复 / 单方 reviewer-codex 出结论 / 稍后重试 / abort）。**严禁**降级同源双 Codex。**合规兜底**（仍异构）：lead 自己用当前 adapter 的 Bash/shell 起外部 claude CLI（按 user 全局模板 `~/.claude/templates/reviewer-claude.sh.tmpl` 填），与 reviewer-codex teammate 仍构成 Opus 4.7 vs gpt-5.5 异构对；reviewer-codex teammate 不动继续按 SKILL 流程跑；lead 拿到独立 claude stdout 后照常做三态裁决。|
