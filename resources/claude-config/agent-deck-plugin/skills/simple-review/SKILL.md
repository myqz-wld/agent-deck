...
Use `kind: 'prompt'` as the expected validation pass after material prompt-asset edits. Do not skip it only because reviewer replies arrive in a later turn; dispatch the pair, stop the turn, and continue adjudication when replies arrive.
...
- ✅ **True issue**: both reviewers independently report it, or one reviewer reports it and lead verifies it with Read/Grep/Bash/test evidence.
