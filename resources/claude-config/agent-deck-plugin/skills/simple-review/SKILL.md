...
| 3 | 三态裁决（§三态裁决）：双方一致 → ✅；单方独有 + HIGH → Step 4 反驳轮；单方独有 + MED → lead 自己 Grep/Read 验证；单方独有 + LOW/INFO → 直接列 ❓ | — |
...
**单方独有分流**：HIGH → §执行模板 Step 4 反驳轮；MED → lead 自己 Grep / Read 验证（≤ 5min / ≤ 5 grep / ≤ 1 test，超就降非 HIGH 不再纠缠）；LOW/INFO → 直接列 ❓。双方都说没问题 → ✅ 可合。
...
本 SKILL = **单次** review + 可选一轮 fix（轻量异构对抗）；`agent-deck:deep-review` = **多轮**挖深 review × fix × 反驳轮（teammate 跨轮持久化）。两个 SKILL 都各自 inline §三态裁决 + §Finding 输出契约（各自 SSOT，不交叉引用应用 CLAUDE.md）。本 SKILL 跑完发现水比预期深（≥ 1 HIGH 反复 / 跨多模块）→ 升 deep-review 接管。
