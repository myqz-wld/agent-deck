...
| 6 | 收尾：`shutdown_session` × 2 + cleanup auto-cp cache（按 manifest 精确 rm，try/finally）+ 输出 §Final Summary Report | shutdown 不删 events / messages，lead 仍可在裁决报告引用。收尾后必须给用户总结报告，不要只说 review 已完成。 |
