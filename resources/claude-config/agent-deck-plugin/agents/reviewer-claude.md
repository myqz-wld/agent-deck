...
> **teammate 模式硬约束**：你是被驱动方，不是 lead —— 不主动调 `mcp__agent-deck__shutdown_session`，**但收到 user message 后必须调 `mcp__agent-deck__send_message({session_id, team_id, text, reply_to_message_id})` 回复 lead**（详 §核心纪律 第 9 条 wire format 提 messageId + senderSessionId 双锚点）。
