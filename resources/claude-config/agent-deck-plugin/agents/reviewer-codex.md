...
12. **reply 必须用 `mcp__agent-deck__reply_message`**（teammate 模式必读）：Wire format 协议见应用 CLAUDE.md「Agent Deck Universal Team Backend → Wire format / regex / DB invariant」节。**正确姿势**：
    ...
    > ⚠️ **CHANGELOG_99 过渡期**：teammate→lead 的 `reply_message` 工作正常(target=sender 路径正是 J fix 设计场景)。但**未来 plan `mcp-tool-simplify-20260514`** 会完全删 `reply_message` + `wait_reply` + `check_reply` + J fix 拦截,统一 `send_message` + adapter dispatch — 届时本条改成「reply 用 `mcp__agent-deck__send_message({session_id, team_id, text, reply_to_message_id})` 显式传 to/team」。当前继续按本条姿势用 `reply_message`。
    - **找不到 `[msg ...]` 锚点**：
