...
# ⚠️ 不在这里 cat / rm —— 模板用 run_in_background: true 跑，命令体里 cat 的 stdout 也在 task output 里，但 $OUT / $ERR 文件本身需保留给后续 Read 工具读，clean-up 留给 wrapper（Read 完后调 `rm -f "$OUT" "$PROMPT" "$ERR"`，或干脆不清——`$TMPDIR` 系统重启自动清）
```
